	...
	<style>
		...
		.ui-button-text-only {
			height:2.1em !important;
						...
						<form id="resultFrm" action="vacationApproval" method="POST">
						...
						<input type="hidden" id="view_type" name="view_type" value="result"/>						
								...
								<table>									
									<thead>
										<tr>											
											...
											<th width="6%">											
												<c:choose>
													<c:when test="${srchApDateOrderTab2 == 'DESC' }"><a href="javascript:doApDateOrderTab2('ASC');" style="color:blue;">결재일▼</a></c:when>
													<c:when test="${srchApDateOrderTab2 == 'ASC' }"><a href="javascript:doApDateOrderTab2('99');" style="color:blue;">결재일▲</a></c:when>
													<c:otherwise><a href="javascript:doApDateOrderTab2('DESC');" style="color:blue;">결재일</a></c:otherwise>
												</c:choose>											
											</th>
											<th width="8%">종류</th>
											<th width="10%">기간</th>
											...
											<c:when test="${approvalList.size() != 0}">
												<c:forEach items="${approvalList}" var="item" varStatus="status">
													<tr>														
														...
														<td class="td-overflow-none"><%-- <fmt:formatNumber value="${item.annual_set}" pattern="0.00"/> --%><fmt:formatNumber value="${item.remain_vacation_use}" pattern="0.00"/> 일</td>
							...
							<input type="hidden" id="srchApDateOrderTab2" name="srchApDateOrderTab2" value="${srchApDateOrderTab2}"/>							
						...
						<script>
							...
							function doApDateOrderTab2( p_order ){
								$("#srchApDateOrderTab2").val( p_order );								
								$("#resultFrm").submit();
							}
							...
							function openVacationView(user_no,user_name,start_date,end_date){
								
								var url = "/vacationUserInfoNewModal";		//사용자 휴가내역 페이지
								var param = {};								
