...
<body>
	...
	<p style="text-align:center;">
		<c:if test="${startPgNoInt1 > 10}">
			<a href="./${pgNm1}?reqPgNo=${startPgNoInt1 - 1}&sDay=${sDay}&eDay=${eDay}">&lt;</a>
		</c:if>

		<c:forEach var="pageNo" begin="${startPgNoInt1}" end="${endPgNoInt1}" step="1">
			<c:choose>
				<c:when test="${reqPgNoInt1 != pageNo}">
					<a href="./${pgNm1}?reqPgNo=${pageNo}&sDay=${sDay}&eDay=${eDay}"><c:out value="${pageNo}" /></a>
					&nbsp;&nbsp;
				</c:when>
				<c:otherwise>
		...
		<c:if test="${totPgCntInt1 >= endPgNoInt1 + 1}">
			<a href="./${pgNm1}?reqPgNo=${endPgNoInt1 + 1}&sDay=${sDay}&eDay=${eDay}">&gt;</a>
		</c:if>
	</p>
