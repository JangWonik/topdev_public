...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
							...
							<div class="tableStyle2">
								<table>
									...
									<tbody>
										<c:forEach items="${boardListTel}" var="boardList">
											<tr>
													<!--번호-->
	...
	<c:if test="${pgNm4 != null}">
		<c:if test="${startPgNoInt4 > 10}">
			<a href="javascript:reqPgNoClick(${startPgNoInt4 - 1},'tel');">&lt;</a>
		</c:if>

		<c:forEach var="pageNo" begin="${startPgNoInt4}" end="${endPgNoInt4}" step="1">
			<c:choose>
				<c:when test="${reqPgNoInt4 != pageNo}">
					<a href="javascript:reqPgNoClick(${pageNo},'tel');"><c:out value="${pageNo}" /></a>
					&nbsp;&nbsp;
				</c:when>
				<c:otherwise>
		...
		<c:if test="${totPgCntInt4 >= endPgNoInt4 + 1}">
			<a href="javascript:reqPgNoClick(${endPgNoInt4 + 1},'tel');">&gt;</a>
		</c:if>
	</c:if>
										<!-- //page -->
									</td>
