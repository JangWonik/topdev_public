...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
							...
							<div class="tableStyle2">
								<table>
									...
									<tbody>
										<c:forEach items="${boardListIntra}" var="boardList">
											<tr>
													<!--번호-->
									...
									<td align="center">
										...
										<c:if test="${pgNm2 != null}">
											<c:if test="${startPgNoInt2 > 10}">
												<a href="javascript:reqPgNoClick(${startPgNoInt2 - 1},'intra');">&lt;</a>
											</c:if>
									
											<c:forEach var="pageNo" begin="${startPgNoInt2}" end="${endPgNoInt2}" step="1">
												<c:choose>
													<c:when test="${reqPgNoInt2 != pageNo}">
														<a href="javascript:reqPgNoClick(${pageNo},'intra');"><c:out value="${pageNo}" /></a>
														&nbsp;&nbsp;
													</c:when>
													<c:otherwise>
											...
											<c:if test="${totPgCntInt2 >= endPgNoInt2 + 1}">
												<a href="javascript:reqPgNoClick(${endPgNoInt2 + 1},'intra');">&gt;</a>
											</c:if>
										</c:if>
										<!-- //page -->
									</td>
