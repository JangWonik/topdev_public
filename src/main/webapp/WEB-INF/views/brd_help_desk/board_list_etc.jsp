...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
							...
							<div class="tableStyle2">
								<table>
									...
									<tbody>
										<c:forEach items="${boardListEtc}" var="boardList">
											<tr>
													<!--번호-->
	...
	<c:if test="${pgNm6 != null}">
		<c:if test="${startPgNoInt6 > 10}">
			<a href="javascript:reqPgNoClick(${startPgNoInt6 - 1},'etc');">&lt;</a>
		</c:if>

		<c:forEach var="pageNo" begin="${startPgNoInt6}" end="${endPgNoInt6}" step="1">
			<c:choose>
				<c:when test="${reqPgNoInt6 != pageNo}">
					<a href="javascript:reqPgNoClick(${pageNo},'etc');"><c:out value="${pageNo}" /></a>
					&nbsp;&nbsp;
				</c:when>
				<c:otherwise>
		...
		<c:if test="${totPgCntInt6 >= endPgNoInt6 + 1}">
			<a href="javascript:reqPgNoClick(${endPgNoInt6 + 1},'etc');">&gt;</a>
		</c:if>
	</c:if>
										<!-- //page -->
									</td>
