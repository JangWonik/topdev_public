...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
							...
							<div class="tableStyle2">
								<table>
									...
									<tbody>
										<c:forEach items="${boardListSecu}" var="boardList">
											<tr>
													<!--번호-->
	...
	<c:if test="${pgNm5 != null}">
		<c:if test="${startPgNoInt5 > 10}">
			<a href="javascript:reqPgNoClick(${startPgNoInt5 - 1},'secu');">&lt;</a>
		</c:if>

		<c:forEach var="pageNo" begin="${startPgNoInt5}" end="${endPgNoInt5}" step="1">
			<c:choose>
				<c:when test="${reqPgNoInt5 != pageNo}">
					<a href="javascript:reqPgNoClick(${pageNo},'secu');"><c:out value="${pageNo}" /></a>
					&nbsp;&nbsp;
				</c:when>
				<c:otherwise>
		...
		<c:if test="${totPgCntInt5 >= endPgNoInt5 + 1}">
			<a href="javascript:reqPgNoClick(${endPgNoInt5 + 1},'secu');">&gt;</a>
		</c:if>
	</c:if>
										<!-- //page -->
									</td>
