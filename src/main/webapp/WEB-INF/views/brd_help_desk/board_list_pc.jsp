...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
							...
							<div class="tableStyle2">
								<table>
									...
									<tbody>
										<c:forEach items="${boardListPc}" var="boardList">
											<tr>
													<!--번호-->
	...
	<c:if test="${pgNm3 != null}">
		<c:if test="${startPgNoInt3 > 10}">
			<a href="javascript:reqPgNoClick(${startPgNoInt3 - 1},'pc');">&lt;</a>
		</c:if>

		<c:forEach var="pageNo" begin="${startPgNoInt3}" end="${endPgNoInt3}" step="1">
			<c:choose>
				<c:when test="${reqPgNoInt3 != pageNo}">
					<a href="javascript:reqPgNoClick(${pageNo},'pc');"><c:out value="${pageNo}" /></a>
					&nbsp;&nbsp;
				</c:when>
				<c:otherwise>
		...
		<c:if test="${totPgCntInt3 >= endPgNoInt3 + 1}">
			<a href="javascript:reqPgNoClick(${endPgNoInt3 + 1},'pc');">&gt;</a>
		</c:if>
	</c:if>
										<!-- //page -->
									</td>
