	...
	<c:choose>		
		...
		<c:otherwise>
			<c:forEach var="items" items="${memberList}" varStatus="status">
				<tr onmouseover="this.style.backgroundColor='#FFECEC'" onmouseout="this.style.backgroundColor=''" onclick="javascript:fnMemberInfoModal( '${items.skey}', '${items.user_name}' )" style="cursor:pointer;">
					...
					<td width="80px;">${items.workload_cnt}</td>
					<td width="80px;">${items.workload_per}%</td>
					<td width="80px;">${items.base_date_fmt}</td>					
					<td width="80px;" title="${items.manager_name}">						
						<c:choose>
							<c:when test="${fn:length(items.manager_name) > 6}">
								${fn:substring(items.manager_name,0,6)}..
							</c:when>
							<c:otherwise>${items.manager_name}</c:otherwise>
						</c:choose>								
					</td>
