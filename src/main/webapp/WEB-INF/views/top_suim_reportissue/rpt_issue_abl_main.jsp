...
<script type="text/javascript">
    ...
    function fnReportWord(){
        ...
        window.open('/reportIssueAbl?suim_rpt_no='+'${param.suim_rpt_no}'+'&cmd=word'+'&suim_rpt_type1='+'${suimVO.suim_rpt_type1}','word','width='+sw+',height='+sh+',top='+mt+',left='+ml+',resizable=no,scrollbars=yes');
    }
    ...
    function fnReportPrint(){
        ...
        window.open('/reportIssueAbl?suim_rpt_no='+'${param.suim_rpt_no}'+'&cmd=print'+'&suim_rpt_type1='+'${suimVO.suim_rpt_type1}','print','width='+sw+',height='+sh+',top='+mt+',left='+ml+',resizable=no,scrollbars=yes');
    }  
    ...
    function fnSendReportPrint(){		//손해사정서 교부 대상자 보고서    	
		...
		window.open("/reportIssueAbl?suim_rpt_no=${param.suim_rpt_no}&cmd=report&suim_rpt_type1=${suimVO.suim_rpt_type1}&ptnr_mbr_nm="+ptnr_nm,'print','width='+sw+',height='+sh+',top='+mt+',left='+ml+',resizable=no,scrollbars=yes');
    }
    ...
    function fnIssueContractSave() {					//보험계약사항 수정 저장
        ...
        var url = "./rptissue_contract_update";
    ...
    //Abl 용 함수 추가 시작
    function fnCommentEdit(){
        $("#btn_comment_edit").hide();
        $("#btn_comment_save").show();
        $("#btn_comment_cancel").show();                

        $("#table_comment_view").hide();
        $("#table_comment_edit").show();
        
    }
    
    function fnCommentCancel() {
        $("#btn_comment_edit").show();
        $("#btn_comment_save").hide();
        $("#btn_comment_cancel").hide();

        $("#table_comment_view").show();
        $("#table_comment_edit").hide();
    }
    ...
    function fnCommentSave() {
        var param = {};
        param.suim_rpt_no       			= $("#suim_rpt_no").val();
        param.report_comment	 = $("#report_comment").val().replace(/\n/g, "<br/>"); 
        param.report_reference = $("#report_reference").val().replace(/\n/g, "<br/>");        
        
        var url = "./rptissue_abl_comment_update";
	...
	//email 양식체크
 	...
 	function fnIssueSendEmail(idx){		
 		...
 		if(confirm("해당 대상자에게 이메일 발송을 하시겠습니까?\n발송 후에는 취소는 불가능합니다.\n발송실패시 이메일 재등록후 발송하세요.")){
             ...
             var url = "./report_Issue_send_abl_email";
      ...
      function fnIssueSendMsg(idx){
          if(confirm("해당 대상자에게 메시지 발송을 하시겠습니까?\n발송실패로 인한 재발송 시에는 연락처 수정 후 \n수정버튼을 클릭하시고 문자발송하세요.")){
              ...
              var url = "./report_Issue_send_abl_msg";
...
<div class="reportWrap">
    <div class="issue_reportContent">
        ...
        <div class="text-right">
            <a href="/reportIssueAbl?suim_rpt_no=${param.suim_rpt_no}&cmd=word&suim_rpt_type1=${suimVO.suim_rpt_type1}"><img src="./resources/ls_img/report/btn_word.gif"></a>
        ...
        <div class="display-table" style="margin-top: 20px;">
            <div class="display-cell">
                <p class="subtitle" style="width: 440px;">3. 손해사정의견</p>		                
            </div>		       		            
            <div class="display-cell text-right">
                <a id="btn_comment_edit" href="#noloc" onclick="fnCommentEdit();"><img src="./resources/ls_img/btn_edite.gif"/></a>
                <a id="btn_comment_save" href="#noloc" style="display: none" onclick="fnCommentSave();"><img src="./resources/ls_img/icon_write.gif"/></a>
                <a id="btn_comment_cancel" href="#noloc" onclick="fnCommentCancel();" style="display: none"><img src="./resources/ls_img/btn_write_cancel.gif"/></a>
            </div>
        </div>  
        <div id="div_comment" class="display-cell text-left">        
		...
		<div class="text-center">	
			<img src="./resources/ne_img/sign_ceo.jpg" /><br/>		
        </div>
        <br/><br/><br/><br/>
