    ...
    <div class="reportContent">
		...
		<div>
			...
			<colgroup>
				...
				<tbody>
				<tr>
					<td style="text-align:justify;letter-spacing:1px;">Report&nbsp;No.<br/>수&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;신<br/>제&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;목<br/>수&nbsp;임&nbsp;일&nbsp;자<br/></td>
					<td style="text-align:left;letter-spacing:0px;">: ${rptIssueMain.suimAcceptNo}<br/>: ${rptIssueMain.ptnrName}<br/>: 손해사정서 교부 결과 안내 [보험사 사고번호 : ${rptIssueMain.accidentNo}]<br/>: ${rptIssueMain.regDate}</td>
					<td style="text-align:right;letter-spacing:0px;">Date : ${sToday}<%-- ${rptIssue.writeDateFmt} --%></td>
				</tr>				
				</tbody>
			</table>
		</div>
			...
			<table id="table_issue_contract_view" style="border: 1px solid currentColor;">
				...
				<tr>				    
				    ...
				    <td style="text-align: left;padding-left:5px;">
				    	...
				    	<c:if test="${item.sendMethod eq 'email' or item.sendMethod eq 'email_send' }">이메일
							...
							<c:when test="${not empty suimVO.suimRptState}">
				             ...
				             <c:otherwise>
				               	<c:choose>
				                	...
				                	<c:when test="${item.privacyAgree == 1 && (item.sendMethod eq 'email' || item.sendMethod eq 'fax' || item.sendMethod eq 'mobile')}">
										종결후 교부가능
									</c:when>
				                	<c:otherwise>
										-	                    	
				    ...
				    <td>
				    	<c:choose>
							...
							<c:when test="${item.sendState == 1 || ( item.sendMethod eq 'fax' || item.sendMethod eq 'email' || item.sendMethod eq 'mobile' ) }">
							 	...
							 	<table>
							 		<tr>						 			
							 			<td width="12%" style="font-size:14px; font-family: 돋움; color: #000; text-align:left;">
							 				<c:choose>
							 					<c:when test='${certiUserVo.certi_state_val eq "보조인"}'>담당자</c:when>
