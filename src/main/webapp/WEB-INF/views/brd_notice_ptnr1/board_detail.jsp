...
<head>
	...
	<script>
							...
							function(data,status){
								if(status == "success"){
									if(data.length == 0){alert(data);
										...
										window.location.assign("./noticePtnr1BoardList?insu=${searchVO.insu}&boardSrcType=${searchVO.boardSrcType}&boardSrcWord=${searchVO.boardSrcWord}&reqPgNo=${reqPgNo}");
									}//if(data.length == 0){
								}else{
		...
		function boardPrintPop(boardNo){
			...
			window.open('./noticePtnr1BoardPrint?boardNo='+boardNo,'top','width='+sw+',height='+sh+',top='+mt+',left='+ml+',resizable=no,scrollbars=yes');
		}
	</script>
</head>
					...
					<div class="guest">
						...
						<div class="guestcnt4" style="display:block;">
									...
									<tr>
										...
										<td colspan="5">
											<c:forEach items="${boardFileList}" var="boardFileList">
												<p>
													<u>
														<a href="./brdNoticePtnr1FileDownload?fileNo=${boardFileList.board_file_no}">
							...
							<p style="text-align:center;margin-top:10px;">
								<c:if test="${mbrVo_Session.user_no eq boardDetail.user_no}">
									<a href="./noticePtnr1BoardUpdateForm?insu=${boardDetail.ptnr_id}&boardNo=${boardDetail.board_no}&boardSrcType=${searchVO.boardSrcType}&boardSrcWord=${searchVO.boardSrcWord}&reqPgNo=${reqPgNo}"><img src="./resources/ls_img/ls_board/btn_modify.gif" /></a>
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<a href="#"><img src="./resources/ls_img/ls_board/btn_del.gif" id="boardDeleteBtn" /></a>
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								</c:if>
								<a href="./noticePtnr1BoardList?insu=${boardDetail.ptnr_id}&boardSrcType=${searchVO.boardSrcType}&boardSrcWord=${searchVO.boardSrcWord}&reqPgNo=${reqPgNo}"><img src="./resources/ls_img/ls_board/btn_list.gif" /></a>
							</p>
							<br />
							<!-- memo -->
							<input type="hidden" id="targetStr" value="noticePtnr1" />
