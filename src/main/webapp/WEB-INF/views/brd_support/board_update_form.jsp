		...
		$(document).ready(function(){
			...
			$(".fileDelBtn").click(function(){
				...
				if(tmp == false){
					...
					$.post("./supportTopBoardFileDelete",
				...
				<div class="centercnt_bot1-2">
					<div class="guest">
						...
						<div class="guestcnt4" style="display:block;">
							<form id="uploadForm" name="uploadForm" action="./supportTopBoardUpdate" method="post" enctype="multipart/form-data">
								...
								<div class="tableStyle21">
									<table>
										...
										<tr>
											<td width="15%" bgcolor="#e7ecf1" style="text-align:center;">분 류</td>
											<td style="text-align:left;padding-left:10px;">												
												<select id="category_id" name="category_id" style="width:95%;">
													<c:forEach items="${categoryList}" var="categoryVO">
														<option value="${categoryVO.col_cd}" <c:if test="${boardDetail.category_id == categoryVO.col_cd}">selected</c:if> > ${categoryVO.col_val} </option>
													</c:forEach>
												</select>
											</td>
											<td width="15%" bgcolor="#e7ecf1" style="text-align:center;">중요 글 설정</td>
											<td colspan="3">
												<select id="best_flag" name="best_flag" style="width:50%;">
													<c:choose>
														<c:when test="${boardDetail.best_flag eq 0}">
															<option value="0" selected>사용한함</option>
															<option value="1">사용함</option>
														</c:when>
														<c:otherwise>
															<option value="0">사용한함</option>
							...
							<p style="text-align:right;margin-top:10px;">								
								...
								<a href="./supportTopBoardDetail?boardNo=${boardDetail.board_no}&boardSrcType=${searchVO.boardSrcType}&boardSrcWord=${searchVO.boardSrcWord}&cateSearch=${searchVO.cateSearch}&reqPgNo=${reqPgNo}" class="btn-vacation-aprv" style="background:#F6CED8;">취소</a>
							</p>
						</div>
					</div>
				</div>
