...
<head>
	...
	<script>
							...
							function(data,status){
								if(status == "success"){
									if(data.length == 0){alert(data);
										...
										window.location.assign("./supportTopBoardList?boardSrcType=${searchVO.boardSrcType}&boardSrcWord=${searchVO.boardSrcWord}&titleSearch=${searchVO.titleSearch}&reqPgNo=${reqPgNo}");
									}//if(data.length == 0){
								}else{
		...
		function boardPrintPop(boardNo){
			...
			window.open('./supportTopBoardPrint?boardNo='+boardNo,'top','width='+sw+',height='+sh+',top='+mt+',left='+ml+',resizable=no,scrollbars=yes');
		}
	</script>
</head>
			...
			<div class="center_cnt">
				...
				<div class="centercnt_bot1-2">
					<div class="guest">
						<div class="guestTap">
							<ul>
								<li class="on"><a href="#">전산지원FAQ</a></li>
							</ul>
						</div>
						<div class="guestcnt4" style="display:block;">
							...
							<div class="tableStyle21">
								<table>
									<tr>
										<td width="15%" bgcolor="#e7ecf1" style="text-align:center;">분류</td>
										<td width="25%">${boardDetail.category_id_nm}</td>
										<td width="15%" bgcolor="#e7ecf1" style="text-align:center;">작성자</td>
										<td width="25%">${boardDetail.user_nm} ( ${boardDetail.reg_date_fmt} )</td>
									...
									<tr>
										...
										<td colspan="5">
											<c:forEach items="${boardFileList}" var="boardFileList">
												<p>
													<u>
														<a href="./brdSupportTopFileDownload?fileNo=${boardFileList.board_file_no}">
							...
							<p style="text-align:right;margin-top:10px;">
								<c:if test="${mbrVo_Session.user_state eq '9'}">																		
									<a href="./supportTopBoardUpdateForm?boardNo=${boardDetail.board_no}&boardSrcType=${searchVO.boardSrcType}&boardSrcWord=${searchVO.boardSrcWord}&cateSearch=${searchVO.cateSearch}&reqPgNo=${reqPgNo}"  class="btn-vacation-aprv" style="background:#CEE3F6;">수정</a>
									<a href="#noloc" class="btn-vacation-aprv" id="boardDeleteBtn" style="background:#F6CED8;">삭제</a>
								</c:if>								
								<a href="./supportTopBoardList?boardSrcType=${searchVO.boardSrcType}&boardSrcWord=${searchVO.boardSrcWord}&cateSearch=${searchVO.cateSearch}&reqPgNo=${reqPgNo}" class="btn-vacation-aprv" id="boardSrcBtn">목록</a>
							</p>
							<br />
							<!-- memo -->
							<input type="hidden" id="targetStr" name="targetStr" value="supportTop" />
