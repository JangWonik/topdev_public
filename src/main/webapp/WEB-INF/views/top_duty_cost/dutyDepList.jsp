					...
					<div class="nociteA log_top">
						<div class="search_bar" style="height:25px;">	
							...
							<div style="float: left;">
								...
								<c:choose>
									<c:when test="${srchArg.type eq 'complete'}">
										&nbsp;<b>당직비 입금완료</b>
									</c:when>
									<c:otherwise>
										&nbsp;<b>당직비 지급</b>									
...
<%-- 								<a id="btn_excel" href="#noloc">									
								...
								<span id="yeartxt" href="">${srchArg.yearMonth.substring(0,4)}</span>
						    ...
						    <form id=dutyDepList name="dutyDepList" method="get" action="dutyDepList" onsubmit="return fnSearch();">
								<input type="hidden" id="yearMonth" name="yearMonth" value="${srchArg.yearMonth}">
	                            <input type="hidden" id="pageIdx" name="pageIdx" value="${srchArg.pageIdx}"/>
	                            ...
	                            <input type="hidden" id="retType" name="retType" value="20"/>
	                            <input type="hidden" id="type" name="type" value="${srchArg.type}" /> <!-- 180111, 당직비 지급 / 입금완료 구분 위하여 추가 -->
	                            <input type="hidden" id="excelYN" name="excelYN" /> <!-- 180111, 당직비 지급 / 입금완료 구분 위하여 추가 -->
                            </form>															
				...
				<div class="center_cnt_1300_bot" style="margin:0 auto;">
					<div class="guest">
						...
						<div class="guestTap">
							<ul>								
								...
								<c:if test="${mbrAuthVo_Session.mbr_pms_18 == 1}">
									<li <c:if test="${srchArg.type == 'wait'}"> class="on"</c:if> >
										<a href="#noloc" onclick="location.href='./dutyDepList?type=wait'">당직비 지급</a>
									</li>	
									<li <c:if test="${srchArg.type == 'complete'}"> class="on"</c:if> >
										<a href="#noloc" onclick="location.href='./dutyDepList?type=complete'">당직비 입금완료</a>
									</li>	
								</c:if>
                        ...
                        <input type="checkbox" id="checkall" onclick="fnCheckAll();">
						<c:if test="${mbrAuthVo_Session.mbr_pms_18 == '1'}">                      
<!-- 	                        <input type="button" onclick="fnAprvUpdateAll('A');" value="입금 승인"/> -->
<!-- 	                        <input type="button" onclick="fnAprvUpdateAll('C');" value="입금 승인 취소"/> -->
							<a href="#" class='ClassButton' onclick="fnAprvUpdateAll('A');">입금승인</a>
							<a href="#" class='ClassButton' onclick="fnAprvUpdateAll('C');">승인취소</a> 
                        </c:if> 
						<div id="div_duty" style="margin-top: 10px;">
							<div class="tableStyle2">
                                <form id="listFrm" name="listFrm" action="/dutyCostStatusAllUpdate">
                                    <input type="hidden" id="listCmd" name="cmd" value="">
									<table>
								 	<colgroup>
											<col width="5%">
											<col width="5%">
									...
									<c:forEach var="item" items="${list}" varStatus="status">
										...
										<tr>
											...
											<td><input type="checkbox" id="dutyId_chk_${status.index}" name="dutyId" value="${item.dutyId}"/></td>
                                            <td>${pageInfo.totalRecordCount - (pageInfo.recordCountPerPage * (pageInfo.currentPageNo-1)) - status.index}</td>
											<td><a href="/dutyCostView?dutyId=${item.dutyId}">${fn:substring(item.dutyId,0,7)}-${fn:substring(item.dutyId,7,9)}</a></td>
											...
											<td>
												<c:choose>
													<c:when test="${item.dutyStatus == '10'}">
														<c:if test="${mbrAuthVo_Session.mbr_pms_22 == '1'}">
															<a href="#" class='ClassButton' onclick="fnAprvUpdate('${item.dutyId}','20', 'A');"><img alt="결재대기" src="/resources/ls_img/appwait.png" style = "width: 100%; max-width: 54px;"></a>
														</c:if>
													...
													<c:when test="${item.dutyStatus == '20'}">
														<c:if test="${mbrAuthVo_Session.mbr_pms_18 == '1'}">
															<a href="#" class='ClassButton' onclick="fnAprvUpdate('${item.dutyId}','30','A');">입급대기</a>
														</c:if>
														<c:if test="${mbrAuthVo_Session.mbr_pms_22 == '1'}">
													...
													<c:when test="${item.dutyStatus == '30'}">
														<!-- <img alt="입금완료" src="/resources/ls_img/depcomple.png" style = "width: 100%; max-width: 54px;"> -->
															<a href="#" class='ClassButton'>입금완료</a>
														<c:if test="${mbrAuthVo_Session.mbr_pms_18 == '1'}">
...
<script type="text/javascript">
    ...
    function fnGoPage(pageIdx) {
        var frm = document.getElementById("dutyDepList");
        frm.method = "get";
        frm.action = "./dutyDepList";
        $("#pageIdx").val(pageIdx);
        $("#excelYN").val("N");
        frm.submit();
    }
    ...
    function fnAprvUpdateAll(cmd){

        var chkCnt = $("input[id^='dutyId_']:checked").length;
        ...
        frm.method = "post";
        frm.action = "./dutyCostStatusAllUpdate";
        frm.submit();
    }

    function fnCheckAll(){
        var checkObjs = $("input[id^='dutyId_']");
