...
<head>
	...
	<c:set value="${suimSuit12List}" var="suimSuit12List"/>
	<script>
		...
		$(document).ready(function(){
			...
			$("#excelBtn").click(function(){
				...
				$("#suit12SearchForm").attr("action","topSuimSuit12List_Excel");
				$("#suit12SearchForm").submit();
			});
			/***** searchBtn *****/
			$("#searchBtn").click(function(){
				$("#hiddenFrom").val($("#selectorFrom").val());
				$("#hiddenTo").val($("#selectorTo").val());
				$("#suit12SearchForm").attr("action","topSuimSuit12List");
				$("#suit12SearchForm").submit();
			});
			
			/* 응답전문 */
			$("#Suit12ListRs").click(function(){
	...
	<script>
		function suit12RsList(){
			...
			var ml=(cw-sw)/2;   //가운데 띄우기위한 창의 x위치
			var mt=(ch-sh)/2;   //가운데 띄우기위한 창의 y위치
			window.open('suit12RsList?viewFromDate='+$("#selectorFrom").val()+'&viewToDate='+$("#selectorTo").val(),'suit12RsList','width='+sw+',height='+sh+',top='+mt+',left='+ml+',resizable=no,scrollbars=yes');
		}//suit12Rs
	</script>
	<style>
		.tableStyle2 td{
			...
			<div class="center_cnt_1300">
				...
				<div class="center_cnt_1300_bot">
					<div class="guest">
						<div class="guestTap">
							<ul>
								<li class="on"><a href="#">적부(한화)</a></li>
							</ul>

							<form id="suit12SearchForm" method="GET">
							...
							<p align="right" style="margin-top:5px;">종결일 : 
								...
								<img src="./resources/ls_img/report/icon_contract_rs.gif" id="Suit12ListRs" style="cursor:pointer;"/>
						...
						<div class="guestcnt4" style="display:block;">
							<div class= "tableStyle2">
								<table>									
									...
									<tbody>
										<!--  -->
										<c:forEach items="${suimSuit12List}" var="suit12VO" > <!-- begin="1" end="20" step="1" -->
										...
										style ="cursor:pointer;" onclick="popSuimSuit12Dtl('${suit12VO.suimRptNo}', '${suit17VO.delDate}')">
											...
											<!-- 소재지 주소 -->
											<%-- <td title="${suit12VO.rqBld_CtnAdr}" style="text-align:left;padding-left:5px;">
												<c:choose>
													<c:when test="${fn:length(suit12VO.rqBld_CtnAdr) > 18}">
														${fn:substring(suit12VO.rqBld_CtnAdr,0,18)}...
													</c:when>
													<c:otherwise>${suit12VO.rqBld_CtnAdr}</c:otherwise>
												</c:choose>																									
											</td> --%>
											<!-- 영위 업종 -->
											<td title="${suit12VO.rqBld_MngmBnNm}" style="text-align:left;padding-left:5px;">
												<c:choose>
													<c:when test="${fn:length(suit12VO.rqBld_MngmBnNm) > 28}">
														${fn:substring(suit12VO.rqBld_MngmBnNm,0,28)}...
													</c:when>
													<c:otherwise>${suit12VO.rqBld_MngmBnNm}</c:otherwise>
												</c:choose>																									
											</td>											
											<!-- 계약자 -->
											<td title="${suit12VO.policyholderNm}">
												<c:choose>
											...
											<td>
												<%-- <p <c:if test="${suit12VO.suimRptState  eq 3}">title="${suit12VO.suim_cancelDate_fmt}"</c:if> > --%>
													<c:choose>
														...
														<c:when test="${suit12VO.suimRptState  eq 1}"><font color="blue">결재</font></c:when>
