				...
				<div class="centercnt_bot1-2">
					<div class="guest">
						...
						<div class="guestcnt4" style="display:block;">
							<form id="uploadForm" name="uploadForm" action="./refReportBoardUpdate" method="post" enctype="multipart/form-data">
								...
								<input type="hidden" id="category_id" name="catagory_id" value="${boardDetail.category_id}"/>
												...
												<select id="eventSelect" name="eventSelect" style="width:90%;">
													<c:forEach items="${eventList}" var = "eventVo">
														<option value="${eventVo.col_cd}"
															<c:if test="${boardDetail.event_id == eventVo.col_cd}">selected</c:if> > ${eventVo.col_val} </option>
													</c:forEach>
												</select>
											</td>
											<td width="15%" bgcolor="#e7ecf1" style="text-align:center;">유 형</td>
											<td colspan="3" style="text-align:left;padding-left:10px;">
												<select id="typeSelect" name="typeSelect" style="width:50%;">												
													<c:forEach items="${typeList}" var = "typeVo">
														<option value="${typeVo.col_cd}"
															<c:if test="${boardDetail.type_id == typeVo.col_cd}">selected</c:if> > ${typeVo.col_val} 
														</option>													
													</c:forEach>
												</select>
											</td>
							...
							<p style="text-align:right;margin-top:10px;">								
								...
								<a href="./refReportBoardDetail?boardNo=${boardDetail.board_no}&cateSearch=${searchVO.cateSearch}&eventSearch=${searchVO.eventSearch}&typeSearch=${searchVO.typeSearch}&boardSrcWord=${searchVO.boardSrcWord}&reqPgNo=${reqPgNoInt1}" class="btn-vacation-aprv" style="background:#F6CED8;">취소</a>
							</p>
						</div>
					</div>
				</div>
