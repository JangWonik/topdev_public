...
<c:set var="ConS_MList" value="${ConS_MList}" />
...
<c:set var="ConS_sum4" value="${ConS_sum4}" />
				...
				<div class="centercnt_top">
					<div class="nociteA log_top">
						<div class="search_bar">
							<%@ include file="contract_member_top.jsp" %>
						</div>
					</div>
				</div>
				...
				<tbody>
					...
					<c:forEach var="vo" items="${ConS_MList}" varStatus="status">
				<tr onMouseOver=this.style.backgroundColor="#FFECEC" onMouseOut=this.style.backgroundColor="">
					
					<td>
						<a href="./contract_member2?team_name=${team_name}&user_id=${vo.getUser_id()}&name=${vo.getName()}&ViewFromDate=${fromdate}&ViewToDate=${todate}">
				...
				<tr>
					...
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${ConS_Msum.getSum_reg_m()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${ConS_Msum.getSum_reg_y()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${ConS_Msum.getSum_endok_m()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${ConS_Msum.getSum_endok_y()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${ConS_Msum.getSum_endno_m()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${ConS_Msum.getSum_endno_y()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${ConS_Msum.getSum_end_sum_m()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${ConS_Msum.getSum_end_sum_y()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${ConS_Msum.getSum_rid_mi()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${ConS_Msum.getSum_price_g()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${ConS_Msum.getSum_price_f()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${ConS_Msum.getSum_price_sum()}"/>
					</td>
