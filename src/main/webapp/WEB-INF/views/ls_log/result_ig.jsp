...
<c:set var="ReusltIgList4" value="${ReusltIgList4}" />
<c:set var="R_Ig4List" value="${R_Ig4List}" />


				<div class="centercnt_top">
					<div class="nociteA log_top">
						<div class="search_bar">
							<%@ include file="result_ig_top.jsp" %>
						</div>
					</div>
				</div>
				...
				<div class="centercnt_bot2">
					<div class="guest">
						<div class="guestTap">
							<ul>
									<li class="on"><a>${toMonthInt}월 보험사별 처리내용</a></li>
							</ul>
						</div>
	...
	<tr>
		...
		<td class="td" bgcolor="#EFEFEF"><p><b>보험사</b></p></td>
	...
	<tbody>
		...
		<c:forEach var="vo" items="${R_Ig4List}" varStatus="status">
	<tr onMouseOver=this.style.backgroundColor="#FFECEC" onMouseOut=this.style.backgroundColor="">
		<c:if test="${status.index == 0}">
			<td class="td" bgcolor="#F8F8F8" rowspan="${R_Ig4List.size()}">
				<c:out value="4종" />
			</td>
		</c:if>
		<td>
			<a href="./result_ig_team?ig_nick=${vo.getIg_nick()}&ig_id=${vo.getIg_id()}&ViewFromDate=${fromdate}&ViewToDate=${todate}">
	...
	<tr>
		...
		<td class="td" bgcolor="#F8F8F8"><font color="red">
			<c:out value="${ResultIgSumVO.getSum_report_result1()}"/><br>
		</td>
		<td class="td" bgcolor="#F8F8F8"><font color="red">
			<c:out value="${ResultIgSumVO.getSum_report_result2()}"/><br>
		</td>
		<td class="td" bgcolor="#F8F8F8"><font color="red">
			<c:out value="${ResultIgSumVO.getSum_report_result3()}"/><br>
		</td>
		<td class="td" bgcolor="#F8F8F8"><font color="red">
			<c:out value="${ResultIgSumVO.getSum_report_result4()}"/><br>
		</td>
		<td class="td" bgcolor="#F8F8F8"><font color="red">
			<c:out value="${ResultIgSumVO.getSum_report_result5()}"/><br>
		</td>
		<td class="td" bgcolor="#F8F8F8"><font color="red">
			<c:out value="${ResultIgSumVO.getSum_report_result6()}"/><br>
		</td>
		<td class="td" bgcolor="#F8F8F8"><font color="red">
			<c:out value="${ResultIgSumVO.getSum_report_result7()}"/><br>
		</td>
		<td class="td" bgcolor="#F8F8F8"><font color="red">
			<c:out value="${ResultIgSumVO.getSum_report_result8()}"/><br>
		</td>
		<td class="td" bgcolor="#F8F8F8"><font color="red">
			<c:out value="${ResultIgSumVO.getSum_report_result9()}"/><br>
		</td>
		<td class="td" bgcolor="#F8F8F8"><font color="red">
			<c:out value="${ResultIgSumVO.getSum_report_result_sum()}"/><br>
		</td>
