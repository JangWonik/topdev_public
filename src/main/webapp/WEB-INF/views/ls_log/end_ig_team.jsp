...
<c:set var="kind1endigTList" value="${kind1endigTList}" />
<c:set var="endIgT1List" value="${endIgT1List}" />
<c:set var="kind4endigTList" value="${kind4endigTList}" />

				<div class="centercnt_top">
					<div class="nociteA log_top">
						<div class="search_bar">
							<%@ include file="end_ig_team_top.jsp" %>
						</div>
					</div>
				</div>
	...
	<tbody>
		...
		<c:forEach var="vo" items="${kind1endigTList}" varStatus="status">
	<tr onMouseOver=this.style.backgroundColor="#FFECEC" onMouseOut=this.style.backgroundColor="">
		<c:if test="${status.index == 0}">
			<td class="td" bgcolor="#F8F8F8" rowspan="${kind1endigTList.size()}">
				<c:out value="1종" />
			</td>
		</c:if>
		<td>
	...
	<c:forEach var="vo" items="${kind4endigTList}" varStatus="status">
	<tr onMouseOver=this.style.backgroundColor="#FFECEC" onMouseOut=this.style.backgroundColor="">
		<c:if test="${status.index == 0}">
			<td class="td" bgcolor="#F8F8F8" rowspan="${kind4endigTList.size()}">
				<c:out value="4종" />
			</td>
		</c:if>
		<td>
