...
<c:set var="RT4List" value="${RM4List}" />


				<div class="centercnt_top">
					<div class="nociteA log_top">
						<div class="search_bar">
							<%@ include file="result_member_top.jsp" %>
						</div>
					</div>
				</div>
	...
	<tr>
		...
		<td class="td" bgcolor="#EFEFEF"><p><b>이름</b></p></td>
	...
	<tbody>
		...
		<c:forEach var="vo" items="${RM4List}" varStatus="status">
	<tr onMouseOver=this.style.backgroundColor="#FFECEC" onMouseOut=this.style.backgroundColor="">
		<c:if test="${status.index == 0}">
			<td class="td" bgcolor="#F8F8F8" rowspan="${RM4List.size()}">
				<c:out value="${team_name}" />
			</td>
		</c:if>
		<td>
			<a href="./end_member?team_name=${vo.getTeam_name()}&team_id=${vo.getTeam_id()}&year=${toAccumYear}&fromdate=${fromdate}&todate=${todate}">
