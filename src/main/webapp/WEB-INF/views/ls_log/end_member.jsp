...
<c:set var="endMemberList" value="${endMemberList}" />
<c:set var="endMList" value="${endMList}" />
...
<c:set var="team_name" value="${team_name}" />
				...
				<div class="centercnt_top">
					<div class="nociteA log_top">
						<div class="search_bar">
							<%@ include file="end_member_top.jsp" %>
						</div>
					</div>
				</div>
				...
				<div class="centercnt_bot2">
					<div class="guest">
						<div class="guestTap">
							<ul>
									<li class="on"><a>${toMonthInt}월 ${team_name} 종결현황</a></li>
							</ul>
						</div>
	...
	<tbody>
		...
		<c:forEach var="vo" items="${endMemberList}" varStatus="status">
	<tr onMouseOver=this.style.backgroundColor="#FFECEC" onMouseOut=this.style.backgroundColor="">
		<c:if test="${status.index == 0}">
			<td class="td" bgcolor="#F8F8F8" rowspan="${endMemberList.size()}">
