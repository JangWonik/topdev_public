...
<c:set var="kind1endigList" value="${kind1endigList}" />
<c:set var="end1igList" value="${end1igList}" />
<c:set var="kind4regList" value="${kind4regList}" />

				<div class="centercnt_top">
					<div class="nociteA log_top">
						<div class="search_bar">
							<%@ include file="end_ig_top.jsp" %>
						</div>
					</div>
				</div>
	...
	<tbody>
		...
		<c:forEach var="vo" items="${kind1endigList}" varStatus="status">
	<tr onMouseOver=this.style.backgroundColor="#FFECEC" onMouseOut=this.style.backgroundColor="">
		<c:if test="${status.index == 0}">
			<td class="td" bgcolor="#F8F8F8" rowspan="${kind1endigList.size()}">
				<c:out value="1종" />
			</td>
		</c:if>
		<td>
			<a href="./end_ig_team?ig_id=${vo.getIg_id()}&ig_nick=${vo.getIg_nick()}&year=${toAccumYear}&month=${toMonthInt+monthPlusInt}&fromdate=${fromdate}&todate=${todate}">
	...
	<c:forEach var="vo" items="${kind4endigList}" varStatus="status">
	<tr onMouseOver=this.style.backgroundColor="#FFECEC" onMouseOut=this.style.backgroundColor="">
		<c:if test="${status.index == 0}">
			<td class="td" bgcolor="#F8F8F8" rowspan="${kind4endigList.size()}">
				<c:out value="4종" />
			</td>
		</c:if>
		<td>
