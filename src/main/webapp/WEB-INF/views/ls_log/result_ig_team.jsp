...
<c:set var="ig_id" value="${ig_id}" />
				...
				<div class="centercnt_top">
					<div class="nociteA log_top">
						<div class="search_bar">
							<%@ include file="result_ig_team_top.jsp" %>
						</div>
					</div>
				</div>
	...
	<tbody>
		...
		<c:forEach var="vo" items="${R_Ig_T4List}" varStatus="status">
	<tr onMouseOver=this.style.backgroundColor="#FFECEC" onMouseOut=this.style.backgroundColor="">
		<c:if test="${status.index == 0}">
			<td class="td" bgcolor="#F8F8F8" rowspan="${R_Ig_T4List.size()}">
				<c:out value="4종" />
			</td>
		</c:if>
		<td>
			<a href="./result_ig_member?ig_nick=${vo.getIg_nick()}&ig_id=${ig_id}&team_id=${vo.getTeam_id()}&Team_name=${vo.getTeam_name()}&ViewFromDate=${fromdate}&ViewToDate=${todate}">
