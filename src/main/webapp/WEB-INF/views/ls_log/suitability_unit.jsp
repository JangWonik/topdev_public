...
<c:set var="Suit_UrList" value="${Suit_UrList}" />


				<div class="centercnt_top">
					<div class="nociteA log_top">
						<div class="search_bar">
							<%@ include file="suitability_unit_top.jsp" %>
						</div>
					</div>
				</div>
				...
				<div class="centercnt_bot2">
					<div class="guest">
						<div class="guestTap">
							<ul>
									<li class="on"><a>${toMonthInt}월 개인별 적부심사(KB)</a></li>
							</ul>
						</div>
				...
				<tbody>
					...
					<c:forEach var="vo" items="${Suit_UrList}" varStatus="status">
				...
				<tr>
					...
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${Suit_Usum.getSum_reg_m()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${Suit_Usum.getSum_reg_y()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${Suit_Usum.getSum_endok_m()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${Suit_Usum.getSum_endok_y()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${Suit_Usum.getSum_endno_m()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${Suit_Usum.getSum_endno_y()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${Suit_Usum.getSum_end_sum_m()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${Suit_Usum.getSum_end_sum_y()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<c:out value="${Suit_Usum.getSum_rid_mi()}"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<fmt:formatNumber value="${Suit_Usum.getSum_price_g()}" pattern="#,##0.##"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<fmt:formatNumber value="${Suit_Usum.getSum_price_f()}" pattern="#,##0.##"/>
					</td>
					<td class="td" bgcolor="#EFEFEF">
						<fmt:formatNumber value="${Suit_Usum.getSum_price_sum()}" pattern="#,##0.##"/>
					</td>
