...
<script>
	...
	function chgmember(){
		   ...
		   formM.MselectVal.value=formM.membersel.value;
		   formM.Selname.value=formM.membersel.value;
		}
	...
	<c:set var="tmpStr" value="${tmpStr3}" />
	<c:set var="Selname" value="${Selname}" />
	...
	<c:set var="str" value="${fromdate}"></c:set>
		<b>${toAccumYear}년 ${toMonthInt}월 </b></font><b>${team_name}</b>&nbsp;<b>${Selname}</b> 개인별 통계
	</div>
	...
	<form name="datef" id="formM" method="POST" action="./log_member2?user_id=${user_id}&team_name=${team_name}&team_id=${team_id}&name=${name}"> 
	<!-- 달력 From -->
	...
	<!-- 달력 To -->
		...
		| 	   --%>
			<select name="user_name" id="membersel" onChange="chgmember();">
					<option value = "">-조사자-</option>
				<c:forEach var="vo" items="${member2List}" varStatus="status">
					<option value = "${vo.getUser_id()}" <c:if test="${MselectVal == vo.getUser_id()}"> selected='selected'</c:if>><c:out value="${vo.getName()}"/></option>
					<%-- <option value = "${vo.getName()}" <c:if test="${Selname == vo.getName()}"> selected='selected'</c:if>><c:out value="${vo.getName()}"/></option> --%>		
				</c:forEach>				
			  </select>
			<input type="hidden" name="Selname" value="${Selname}">	  
		  <input type="hidden" name="MselectVal" value="${MselectVal}">
		  <script language='javascript'>document.datef.user_name.value="${MselectVal}";</script>	&nbsp&nbsp
	...
	<br>
	<form action="./log_member2_excel" method="POST" id="excelform">
