...
$(document).ready(function(){
	$("#attchFileSuit12").change(function(){
		$("#suimSuit12RptOrgFileBtn").click();			
	});	
	
	//업로드 갯수 제한 제거 by top3009 (2019.08.13)
	//if($("#fileListSize").val() == "20"){
		//$("#attchFileSuit12").css("display","none");						  
	//}
	
});

function suit12ImgSubDel(imgSerial,suimRptNo,imgName){
	...
	success:function(data) {
		...
		if(gubun == "0"){
			if( (data != "0") && (data !="1") && (data !="9") ){
				$("#suit12FileListBody").append(
				 "<tr id = 'suit12ImgTr_"+data.split("+")[0]+"'>"
				+	"<td width='70%' align='center'>"+data.split("+")[1]+"</td>"
				+	"<td align='center'>"
				+		"<img src='./resources/ls_img/btn_del.gif' id = 'suit12ImgDelBtn_"+data.split("+")[0]+"' onclick=\"suit12ImgSubDel('"+data.split("+")[0]+"','${suimSuit_12Vo.suimRptNo}','"+data.split("+")[1]+"');\" style='cursor:pointer;'/>"
				+	"</td>"
				+"</tr>"  
				);
...
<form name="UploadForm" id="UploadForm" method="POST" action="upload" enctype="multipart/form-data">
	...
	<!-- 파일입력 -->
	<input type="file" name="attchFile" id="attchFileSuit12"  />
	...
	-->
	<input  type="hidden" name = "func" value = "13"/>
	...
	<input  type="hidden" name = "suimRptNo" value = "${getSuimSuit12DtlBsc.suimRptNo}"/>
	
	<!-- 버튼 : 'jquery.form' 플러그인이 버튼의 submit 타입에서만 동작하여 숨겨놓고 file 태그의 값이 변하면 자동클릭 동작으로 구현 -->
	<input style ="display:none; visibility:hidden;" type="submit" id ="suimSuit12RptOrgFileBtn" class="btn" />
</form>
