...
<script>
	$(document).ready(function(){
		$("#attchFileRptHelp_"+"${helpVo.serialNo}").change(function(){
			$("#rptHelpFileBtn_"+"${helpVo.serialNo}").click();	
		});
		
	});

	$("#UploadFileFormRptHelp_"+"${helpVo.serialNo}").ajaxForm({
		  dataType : 'text',
		  success:function(data) {
			  ...
			  if(gubun == "1"){
				  ...
				  if( (data != "0") && (data !="1") && (data !="9") ){
					  ...
					  var fileSerial = $("#"+data).val();

					  var fileName = $("#attchFileRptHelp_"+fileSerial).val();
					  var subfilename = fileName.substr(fileName.lastIndexOf("\\")+1);

					  $("#rptHelpFileList_"+fileSerial).html("");
					  $("#rptHelpFileList_"+fileSerial).append(
        				   	  
	       					 "<a href='fileDownload?key="+fileSerial+"&type=rptHelpFile&suimRptNo="+$("#helpFileParent_"+fileSerial).val()+"'>"
        				    +  	"<font color='blue'>" +subfilename+"</font>"
       					    + "</a>"
	       					+   "<img src='./resources/ls_img/btn_del_s.gif' onclick=\"helpFileDel('"+fileSerial+"');\" style='cursor:pointer; margin-left:5px;'/>"
	       					
	        		   ); 
					  
					  $("#attchFileRptHelp_"+fileSerial).prop("disabled",true);
...
<form name="UploadFileFormRptHelp" id="UploadFileFormRptHelp_${helpVo.serialNo}" method="POST" action="upload" enctype="multipart/form-data">
	
	<!-- 파일입력 -->
	<c:choose>
		<c:when test="${helpVo.helpFile ne '' }">
			<input type="file" name="attchFile" id="attchFileRptHelp_${helpVo.serialNo}" disabled="disabled" />	
		</c:when>
		<c:otherwise>
			<input type="file" name="attchFile" id="attchFileRptHelp_${helpVo.serialNo}" />
		</c:otherwise>
	</c:choose>
	...
	<input  type="hidden" name = "suimRptNo" id  ="helpFileParent_${helpVo.serialNo}" value = "${helpVo.suimRptNo}"/>
	<input  type="hidden" name = "no" id = "helpFileNo_${helpVo.serialNo}" value = "${helpVo.serialNo}"/>
	
	<!-- 버튼 : 'jquery.form' 플러그인이 버튼의 submit 타입에서만 동작하여 숨겨놓고 file 태그의 값이 변하면 자동클릭 동작으로 구현 -->
	<input style ="display:none; visibility:hidden;" type="submit" id ="rptHelpFileBtn_${helpVo.serialNo}" class="btn" />
</form>
