...
<script>
	$(document).ready(function(){
		$("#attchFileSuit18").change(function(){
			$("#upbtnSuit18").click();			
		});
		
	});
	
	function suit18ImgSubDel(imgSerial,suimRptNo,imgName){
		  ...
		  success:function(data) {
			  ...
			  if(gubun == "0"){
				  ...
				  if( (data != "0") && (data !="1") && (data !="9") ){
					  ...
					  $("#suit18FileListBody").append(
						
							 "<tr id = 'suit18ImgTr_"+data.split("+")[0]+"'>"
							+	"<td width='70%' align='center'>"+data.split("+")[1]+"</td>"
							+	"<td align='center'>"
							+		"<img src='./resources/ls_img/btn_del.gif' id = 'suit18ImgDelBtn_"+data.split("+")[0]+"' onclick=\"suit18ImgSubDel('"+data.split("+")[0]+"','${suimSuit_18Vo.suimRptNo}','"+data.split("+")[1]+"');\" style='cursor:pointer;'/>"
							+	"</td>"
							+"</tr>"  
...
<form name="UploadForm" id="UploadForm" method="POST" action="upload" enctype="multipart/form-data">
	...
	<!-- 기능 명시 
		...
		value = "14" : suit18 img
	-->
	<input  type="hidden" name = "func" value = "14"/>
	...
	<input type="file" name="attchFile" id="attchFileSuit18" size="50" />
	
	<!-- 버튼 : 'jquery.form' 플러그인이 버튼의 submit 타입에서만 동작하여 숨겨놓고 file 태그의 값이 변하면 자동클릭 동작으로 구현 -->
	<input style ="display:none; visibility:hidden;" type="submit" id ="upbtnSuit18" class="btn" />
</form>
