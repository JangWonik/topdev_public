...
<script>
	$(document).ready(function(){
		$("#attchFilePrimBizRptOrg").change(function(){
			$("#primBizRptOrgFileBtn").click();	
		});
		
	});

	$("#UploadFileFormRptOrg").ajaxForm({
		  dataType : 'text',
		  success:function(data) {
			  ...
			  if(gubun == "1"){
				  ...
				  if( (data != "0") && (data !="1") && (data !="9") ){
					...
					//  alert($("#attchFilePrimBizRptOrg").val());
					//  alert(regDate);
					  var fileName = $("#attchFilePrimBizRptOrg").val();
					  var subfilename = fileName.substr(fileName.lastIndexOf("\\")+1);
 
					  $("#rptOrgFileList").append(
        				   	"<tr id = 'rptFileOrgTr_"+fileSerial+"'>"
	       					+	"<td>"
	       					+ "<a href='primBizFileDownload?key="+fileSerial+"&type=rptOrigin&suimRptNo="+$("#suimRptNo").val()+"'>"
...
<form name="UploadFileFormRptOrg" id="UploadFileFormRptOrg" method="POST" action="upload" enctype="multipart/form-data">
	
	<!-- 파일입력 -->
	<input type="file" name="attchFile" id="attchFilePrimBizRptOrg"  />
	...
	<input  type="hidden" name = "suimRptNo" value = "${primBizRptDtl.suim_rpt_no}"/>
	
	<!-- 버튼 : 'jquery.form' 플러그인이 버튼의 submit 타입에서만 동작하여 숨겨놓고 file 태그의 값이 변하면 자동클릭 동작으로 구현 -->
	<input style ="display:none; visibility:hidden;" type="submit" id ="primBizRptOrgFileBtn" class="btn" />
</form>
