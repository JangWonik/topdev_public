...
<script>
	$(document).ready(function(){
		...
		$("#attchFile").change(function(){
			alert("사진을 저장하시면 현재 작성한 내용이 사라질 수 있습니다.");
			var result = confirm("사진을 저장 하시겠습니까?");
			if(result != 0){
				$("#upbtn").click();
// 				location.reload();
			}else{
				//같은 파일 연속 선택 시, 인식을 하지 않음 ->file의 value 초기화
				  ...
				  if( (data != "0") && (data !="1") && (data !="9") ){
					  ...
					  if(imgpage =="1"){
						  ...
						  $.ajax({
					           type:"post",
					           url:"getimgstream",
					           data: {no:no, func:"0", filename : filename},  // func:"0" => 내사진 
					           success : function(imgstream) {
					        	   if(imgstream != ""){
					        		   $("#myimage").attr("src",imgstream); // $("#기능명")
									}else{
										alert('화면출력 실패');
									}
					           }
