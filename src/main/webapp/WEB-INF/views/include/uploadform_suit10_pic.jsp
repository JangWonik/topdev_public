...
<script>
	$(document).ready(function(){
		$("#attchFileSuit10").change(function(){
			$("#upbtnSuit10").click();			
		});
		
	});
	
	function suit10ImgSubDel(imgSerial,suimRptNo,imgName){
		  ...
		  success:function(data) {
			  ...
			  if(gubun == "0"){
				  ...
				  if( (data != "0") && (data !="1") && (data !="9") ){
					  ...
					  $("#suit10FileListBody").append(
						
							 "<tr id = 'suit10ImgTr_"+data.split("+")[0]+"'>"
							+	"<td width='70%' align='center'>"+data.split("+")[1]+"</td>"
							+	"<td align='center'>"
							+		"<img src='./resources/ls_img/btn_del.gif' id = 'suit10ImgDelBtn_"+data.split("+")[0]+"' onclick=\"suit10ImgSubDel('"+data.split("+")[0]+"','${suimSuit_10Vo.suimRptNo}','"+data.split("+")[1]+"');\" style='cursor:pointer;'/>"
							+	"</td>"
							+"</tr>"  
...
<form name="UploadForm" id="UploadForm" method="POST" action="upload" enctype="multipart/form-data">
	...
	<!-- 기능 명시 
		...
		value = "12" : suit10 img
	-->
	<input  type="hidden" name = "func" value = "12"/>
	...
	<!-- DB 등록시 필요한 파라미터 설정-->
	<input  type="hidden" name = "suimRptNo" value = "${suimSuit_10Vo.suimRptNo}"/>
	
	<input type="file" name="attchFile" id="attchFileSuit10" size="50" />
	
	<!-- 버튼 : 'jquery.form' 플러그인이 버튼의 submit 타입에서만 동작하여 숨겨놓고 file 태그의 값이 변하면 자동클릭 동작으로 구현 -->
	<input style ="display:none; visibility:hidden;" type="submit" id ="upbtnSuit10" class="btn" />
</form>
