...
<script>
	...
	$("#UploadForm_big").ajaxForm({
		  ...
		  success:function(data) {
			  ...
			  var gubun =  document.UploadForm_big.gubun.value;
			  var imgpage =  document.UploadForm_big.imgpage.value;
			  var func =  document.UploadForm_big.func.value;
				  ...
				  if( (data != "0") && (data !="1") && (data !="9") ){
					  ...
					  if(imgpage =="1"){
						  ...
						  $.ajax({
					           type:"post",
					           url:"getimgstream",
					           data: {id:id, func:"3", filename : filename},  // func:"3" => partner 큰 사진 
					           success : function(imgstream) {
					        	   if(imgstream != ""){
					        		   $("#bImg").attr("src",imgstream); // $("#기능명")
									}else{
										alert('화면출력 실패');
									}
					           }
...
<form name="UploadForm_big" id="UploadForm_big" method="POST" action="upload" enctype="multipart/form-data">
	
	<!-- 파일입력 -->
	<input type="file" name="attchFile" id="attchFile_big"/>
	
	<!-- 기능 명시 
		value = "0" : 내정보 > 내사진
		value = "1" : 내정보 > 내Sign
		value = "2" : 보험사 정보 > 작은 이미지
		value = "3" : 보험사 정보 > 큰 이미지  
	-->
	...
	<input style ="display:none; visibility:hidden;" type="submit" id ="upbtn_big" class="btn" />
</form>
