...
<script>
	$(document).ready(function(){
		$("#attchFileMyCostBill").change(function(){
			$("#myCostBillFileBtn").click();	
		});
		
	});

	$("#UploadFileFormMyCostBill").ajaxForm({
		  dataType : 'text',
		  success:function(data) {
			  ...
			  if(gubun == "0"){
				  ...
				  if( (data != "0") && (data !="1") && (data !="9") ){
					  ...
					  var fileName = $("#attchFileMyCostBill").val();
					  var subfilename = fileName.substr(fileName.lastIndexOf("\\")+1);
 
					  $("#myCostBillFileList").append(
...
<form name="UploadFileFormMyCostBill" id="UploadFileFormMyCostBill" method="POST" action="upload" enctype="multipart/form-data">
	
	<!-- 파일입력 -->
	<input type="file" name="attchFile" id="attchFileMyCostBill"  />
	...
	<input  type="hidden" name = "cost_deposit_no" value = "${getMemberCostDepositInfo.cost_deposit_no}"/>
	
	<!-- 버튼 : 'jquery.form' 플러그인이 버튼의 submit 타입에서만 동작하여 숨겨놓고 file 태그의 값이 변하면 자동클릭 동작으로 구현 -->
	<input style ="display:none; visibility:hidden;" type="submit" id ="myCostBillFileBtn" class="btn" />
</form>
