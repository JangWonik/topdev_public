				...
				<div class="centercnt_bot1-2">
					<div class="guest">
						...
						<div class="guestcnt4" style="display:block;">
							<form id="uploadForm" name="uploadForm" action="./referencePtnrBoardWrite" method="post" enctype="multipart/form-data">
								<input type="hidden" id="cateSearch" name="cateSearch" value="${searchVO.cateSearch}"/>
								<div class="tableStyle2">
									<table>
										<tr>
											<td width="15%" bgcolor="#e7ecf1">보 험 사</td>
											<td style="text-align:left;padding-left:7px;">
												<select id="ptnrSelect" name="ptnr_id" style="width:70%;">
													<option value="0" selected>전체</option>
													<c:forEach items="${ptnrList}" var = "ptnrVo">
														<option value="${ptnrVo.ptnr_id}"
															<c:if test="${boardWriteFailVO.ptnrSearch == ptnrVo.ptnr_id}">selected</c:if>
														>
											...
											<td width="15%" bgcolor="#e7ecf1">종 구분</td>
											<td style="text-align:left;padding-left:7px;">
												<select id="titleSelect" name="title_classify" style="width:70%;">
													<c:forEach items="${titleClassifyList}" var = "titleVo">
														<option value="${titleVo.col_cd}"
															<c:if test="${boardWriteFailVO.titleSearch == titleVo.col_cd}">selected</c:if> > ${titleVo.col_val} </option>
													</c:forEach>
												</select>
											</td>
										</tr>
										<tr>
											<td width="15%" bgcolor="#e7ecf1">자 료 종 류</td>
											<td style="text-align:left;padding-left:7px;">
												<select id="typeSelect" name="type_classify" style="width:70%;">
													<option value="0" selected>전체</option>
													<c:forEach items="${typeList}" var = "typeVo">
														<option value="${typeVo.col_cd}"
															<c:if test="${boardWriteFailVO.typeSearch == typeVo.col_cd}">selected</c:if> > ${typeVo.col_val} </option>
													</c:forEach>
												</select>
											</td>
											<td width="15%" bgcolor="#e7ecf1">해 당 연 도</td>
											<td style="text-align:left;padding-left:7px;">
												...
												<select id="yearSelect" name="year_classify" style="width:70%;">
													<option value="0" selected>전체</option>												
										...
										<tr>
											...
											<td colspan="3"><input type="text" id="title" name="title" value="${boardWriteFailVO.title}" style="width:98%;"></td>
										</tr>
										<tr>
							...
							<p style="text-align:right;margin-top:10px;">
								...
								<a href="./referencePtnrBoardList" class="btn-vacation-aprv" style="background:#F6CED8;">취소</a>								
							</p>
						</div>
					</div>
				</div>
