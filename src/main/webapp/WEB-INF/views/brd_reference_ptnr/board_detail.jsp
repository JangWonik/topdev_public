...
<head>
	...
	<script>
							...
							function(data,status){
								if(status == "success"){
									if(data.length == 0){alert(data);
										...
										window.location.assign("./referencePtnrBoardList?boardSrcType=${searchVO.boardSrcType}&boardSrcWord=${searchVO.boardSrcWord}&reqPgNo=${reqPgNo}&ptnrSearch=${searchVO.ptnrSearch}&ptnrGubun=${searchVO.ptnrGubun}&titleSearch=${searchVO.titleSearch}&typeSearch=${searchVO.typeSearch}&yearSearch=${searchVO.yearSearch}&cateSearch=${searchVO.cateSearch}");
									}//if(data.length == 0){
								}else{
		...
		function boardPrintPop(boardNo){
			...
			window.open('./referencePtnrBoardPrint?boardNo='+boardNo,'top','width='+sw+',height='+sh+',top='+mt+',left='+ml+',resizable=no,scrollbars=yes');
		}
	</script>
	<style>
		.buttonMini{
				...
				<div class="centercnt_bot1-2">
					<div class="guest">
						...
						<div class="guestcnt4" style="display:block;">
							...
							<input type="hidden" id="ptnr_id" name="ptnr_id" value="${boardDetail.ptnr_id}">							
							...
							<div class="tableStyle21">
								<table>
									...
									<tr>
										...
										<td width="15%" bgcolor="#e7ecf1" style="text-align:center;">종 구분</td>
										<td colspan="3">
											<c:choose>
									...
									<tr>
										...
										<td><a href="javascript:boardPrintPop(${boardDetail.board_no});"><input class="buttonMiniPrint" type="button" value="인쇄하기"></a></td>
									...
									<tr>
										...
										<td colspan="5">
											<c:forEach items="${boardFileList}" var="boardFileList">
												<p>
													<u>
														<a href="./brdRefPtnrFileDownload?fileNo=${boardFileList.board_file_no}">
							...
							<p style="text-align:right;margin-top:10px;">
								<c:if test="${mbrVo_Session.user_no eq boardDetail.user_no or mbrVo_Session.user_state eq '9'  or mbrVo_Session.user_no eq '215'}">																		
									<a href="./referencePtnrBoardUpdateForm?boardNo=${boardDetail.board_no}&boardSrcType=${searchVO.boardSrcType}&boardSrcWord=${searchVO.boardSrcWord}&reqPgNo=${reqPgNo}&ptnrSearch=${searchVO.ptnrSearch}&ptnrGubun=${searchVO.ptnrGubun}&titleSearch=${searchVO.titleSearch}&typeSearch=${searchVO.typeSearch}&yearSearch=${searchVO.yearSearch}&cateSearch=${searchVO.cateSearch}"  class="btn-vacation-aprv" style="background:#CEE3F6;">수정</a>
									<a href="#noloc" class="btn-vacation-aprv" id="boardDeleteBtn" style="background:#F6CED8;">삭제</a>
								</c:if>								
								<a href="./referencePtnrBoardList?boardSrcType=${searchVO.boardSrcType}&boardSrcWord=${searchVO.boardSrcWord}&reqPgNo=${reqPgNo}&ptnrSearch=${searchVO.ptnrSearch}&ptnrGubun=${searchVO.ptnrGubun}&titleSearch=${searchVO.titleSearch}&typeSearch=${searchVO.typeSearch}&yearSearch=${searchVO.yearSearch}&cateSearch=${searchVO.cateSearch}" class="btn-vacation-aprv" id="boardSrcBtn">목록</a>
							</p>
							<br />
							<!-- memo -->
