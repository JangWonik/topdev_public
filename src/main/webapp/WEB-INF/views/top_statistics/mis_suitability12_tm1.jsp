...
<div class="tableStyle2" id="dataTable">
    ...
    <div style="height:566px; overflow-x: hidden; overflow-y: auto;">
        <table cellpadding="0" cellspacing="0" width="1060" summary="팀 기본통계 1종">
            ...
            <tbody>
            <c:forEach var="vo" items="${getMisSuitability12Tm1}" varStatus="status">
                <tr onMouseOver="this.style.backgroundColor='#FFECEC'" onMouseOut="this.style.backgroundColor=''">
                    <c:if test="${status.index == 0}">
                        <td rowspan="${getMisSuitability12Tm1.size()}">1종</td>
                    </c:if>
                    <td><p><a href="javascript:goMisSuitability12TmMbr('${vo.team_id}','${vo.team_name}','${vo.team_type}');">${vo.team_name}</a></p></td>
            ...
            <c:forEach var="vo" items="${getMisSuitability12Tm4}" varStatus="status">
                <tr onMouseOver="this.style.backgroundColor='#FFECEC'" onMouseOut="this.style.backgroundColor=''">
                    <c:if test="${status.index == 0}">
                        <td rowspan="${getMisSuitability12Tm4.size()}">4종</td>
                    </c:if>
                    <td><p><a href="javascript:goMisSuitability12TmMbr('${vo.team_id}','${vo.team_name}','${vo.team_type}');">${vo.team_name}</a></p></td>
                    <td><fmt:formatNumber value="${vo.reg_m}" pattern="#,##0.##"/></td>
                    <td><fmt:formatNumber value="${vo.reg_y}" pattern="#,##0.##"/></td>
