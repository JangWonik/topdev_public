...
<%
    ...
    response.setHeader("Content-Disposition", "attachment; filename=written_team_"+pageContext.getAttribute("today")+".xls");
    response.setHeader("Content-Description", "JSP Generated Data");
    response.setContentType("application/vnd.ms-excel");
%>
<% pageContext.setAttribute("CRLF", "\r\n"); %>
<% pageContext.setAttribute("LF", "\n"); %>
<div style="float: left;padding:10px 0 10px;"><b>기준일 : 
		&nbsp;${s_base_date}</b>
</div>
<br/>
<c:choose>
		...
		&nbsp;<b>${teams.teamName}</b>
			...
			<c:forEach var="items" items="${writtenList}" varStatus="status">
				<c:if test="${teams.teamId eq items.teamId}">
					<tr>
						<td width="9%" align="center">${items.teamName}</td>
						<td width="8%" align="center">${items.userName}</td>
						<td width="4%" align="center">${items.workLevelCd}</td>
