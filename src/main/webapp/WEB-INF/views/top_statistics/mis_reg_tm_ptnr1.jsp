...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
							...
							<div class="tableStyle2">
								<table cellpadding="0" cellspacing="0" width="990" align="center">
									...
									<tbody>
										<c:forEach var="vo" items="${getMisRegTmPtnr1}" varStatus="status">
											<tr style="bgcolor: #E7ECF1; text-align: center;" onMouseOver="this.style.backgroundColor='#FFECEC'" onMouseOut="this.style.backgroundColor=''">
												<c:if test="${status.index == 0}">
													<td rowspan="${getMisRegTmPtnr1.size() + 1}">물보험</td>
												</c:if>
												<td>${vo.ptnr_nick}</td>
												<td>수임<br /><br />종결</td>
												<td bgcolor="#faebd7">${vo.reg_12+vo.reg_11+vo.reg_10+vo.reg_9+vo.reg_8+vo.reg_7+vo.reg_6+vo.reg_5+vo.reg_4+vo.reg_3+vo.reg_2+vo.reg_1}
													<br /><br />
										...
										<c:forEach var="vo" items="${getMisRegTmPtnr4}" varStatus="status">
											<tr>
												<c:if test="${status.index == 0}">
													<td rowspan="${getMisRegTmPtnr4.size() + 1}">인보험</td>
												</c:if>
												<td>${vo.ptnr_nick}</td>
