...
<head>
	...
	<script>
		...
		$(document).ready(function(){
			...
			$("#user_no").change(function(){
				$("#user_name").val($("#user_no option:selected").text());
			});
		...
		function popSuimDtlPrim(suim_rpt_no){
			var cw=screen.availWidth;
			var ch=screen.availHeight;
			var sw=1010; //띄울 창의 넓이
			var sh=740;  //띄울 창의 높이
					...
					<div class="nociteA log_top">
						<div class="search_bar">
							...
							<div style="float: left;">
								...
								&nbsp;<b>${inVO.team_name} ${inVO.user_name} 통계</b>
							</div>
							...
							<form name="misSearchForm" id="misSearchForm" method="GET" action="./getMisTmBscMbr2">
								...
								<input type="hidden" name="user_no" id="user_no" value="${inVO.user_no}" />
<!-- 								<select name="user_no" id="user_no"> -->
<!-- 									<option value = "0"> - 조 사 자 - </option> -->
<%-- 									<c:forEach var="vo" items="${teamMemberList}" varStatus="status"> --%>
<%-- 										<option value = "${vo.getUser_no()}" <c:if test="${inVO.user_no == vo.getUser_no()}"> selected='selected'</c:if>><c:out value="${vo.getUser_name()}"/></option> --%>
<%-- 									</c:forEach>				 --%>
<!-- 								</select>&nbsp;&nbsp;&nbsp; -->
<!--
...
-->
				...
				<div class="centercnt_bot2" style="margin:0 auto;">
					<div class="guest">
						...
						<div class="guestTap">
							<ul>
								<li class="on"><a href="#">기본통계</a></li>
<!-- 								<li class="on"><a href="#">기본통계 -->
<%-- 									<c:choose> --%>
<%-- 										<c:when test="${getMisTmBscMbr2141.size() == 0 --%>
<%-- 														&& getMisTmBscMbr2142.size() == 0 --%>
<%-- 														&& getMisTmBscMbr2Mi.size() == 0}">(0)</c:when> --%>
<%-- 										<c:otherwise>(${getMisTmBscMbr2141.size() --%>
<%-- 														+ getMisTmBscMbr2142.size() --%>
<%-- 														+ getMisTmBscMbr2Mi.size()})</c:otherwise> --%>
<%-- 									</c:choose> --%>
<!-- 								</a></li> -->
<!-- 								<li class="on"><a href="#">농작물 -->
<%-- 									<c:choose> --%>
<%-- 										<c:when test="${getMisTmBscMbr2Prim1.size() == 0 --%>
<%-- 														&& getMisTmBscMbr2Prim2.size() == 0 --%>
<%-- 														&& getMisTmBscMbr2MiPrim.size() == 0}">(0)</c:when> --%>
<%-- 										<c:otherwise>(${getMisTmBscMbr2Prim1.size() --%>
<%-- 														+ getMisTmBscMbr2Prim2.size() --%>
<%-- 														+ getMisTmBscMbr2MiPrim.size()})</c:otherwise> --%>
<%-- 									</c:choose> --%>
<!-- 								</a></li> -->
<!-- 								<li class="on"><a href="#">적부 -->
<%-- 									<c:choose> --%>
<%-- 										<c:when test="${getMisTmBscMbr2PrimContract1.size() == 0 --%>
<%-- 														&& getMisTmBscMbr2PrimContract2.size() == 0 --%>
<%-- 														&& getMisTmBscMbr2MiPrimContract.size() == 0}">(0)</c:when> --%>
<%-- 										<c:otherwise>(${getMisTmBscMbr2PrimContract1.size() --%>
<%-- 														+ getMisTmBscMbr2PrimContract2.size() --%>
<%-- 														+ getMisTmBscMbr2MiPrimContract.size()})</c:otherwise> --%>
<%-- 									</c:choose> --%>
<!-- 								</a></li> -->
							</ul>
						</div>
						<div class="guestcnt2" style="display:block;">
							<c:import url="./mis_tm_bsc_mbr_21.jsp"></c:import>
						</div>
						<div class="guestcnt2" style="display:none;">
							<c:import url="./mis_tm_bsc_mbr_22.jsp"></c:import>
						</div>
						<div class="guestcnt2" style="display:none;">
							<c:import url="./mis_tm_bsc_mbr_23.jsp"></c:import>
						</div>
