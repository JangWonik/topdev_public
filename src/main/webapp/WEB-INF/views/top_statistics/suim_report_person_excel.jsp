...
<div class="tableStyle2" id="area_excel">
    ...
    <div style="height:654px; overflow-x: hidden; overflow-y: auto; padding-right: 17px;">
        <table cellpadding="0" cellspacing="0" width="1060" align="center" border="1">
            <colgroup>
                <col width="65">
                <col width="65">
            ...
            <c:forEach var="vo" items="${ReportList}" varStatus="status">
                <tr style="bgcolor: #E7ECF1; text-align: center;" onMouseOver="this.style.backgroundColor='#FFECEC'" onMouseOut="this.style.backgroundColor=''">
                    ...
                    <td>
                    	<c:choose>                    		
                    		<c:when test="${vo.conMethod eq 'mobile'}">휴대폰교부</c:when>
                    		<c:when test="${vo.conMethod eq 'fax'}">팩스교부</c:when>
                    		<c:when test="${vo.conMethod eq 'email'}">이메일교부</c:when>
                    		<c:when test="${vo.conMethod eq 'disagree'}">비 동의</c:when>                    		
                    		<c:otherwise>미교부</c:otherwise>                    		
                    	</c:choose>                    
                    </td>
                    <td>
                    	<c:choose>                    		
                    		<c:when test="${vo.insMethod eq 'mobile'}">휴대폰교부</c:when>
                    		<c:when test="${vo.insMethod eq 'fax'}">팩스교부</c:when>
                    		<c:when test="${vo.insMethod eq 'email'}">이메일교부</c:when>
                    		<c:when test="${vo.insMethod eq 'disagree'}">비 동의</c:when>                    		
                    		<c:otherwise>미교부</c:otherwise>                    		
                    	</c:choose>                    
                    </td>
                    <td>
                    	<c:choose>                    		
                    		<c:when test="${vo.claMethod eq 'mobile'}">휴대폰교부</c:when>
                    		<c:when test="${vo.claMethod eq 'fax'}">팩스교부</c:when>
                    		<c:when test="${vo.claMethod eq 'email'}">이메일교부</c:when>
                    		<c:when test="${vo.claMethod eq 'disagree'}">비 동의</c:when>                    		
                    		<c:otherwise>미교부</c:otherwise>                    		
                    	</c:choose>
                    </td>                    
                    <td>                    	
