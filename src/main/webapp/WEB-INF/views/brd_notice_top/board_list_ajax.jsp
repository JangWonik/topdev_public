	...
	<table>							
		<c:choose>
			...
			<c:otherwise>
				...
				<tr>
					<td width="9%">${boardListCnt - ( 15 * (pageIdx-1) ) - status.index}</td>										
					<td width="50%" style="text-align:left;padding-left:10px;letter-spacing:0px;">
						<div style="width:400px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
						<a href="./noticeTopBoardDetail?boardNo=${brd.board_no}&boardSrcType=${searchVO.boardSrcType}&boardSrcWord=${searchVO.boardSrcWord}&titleSearch=${searchVO.titleSearch}&reqPgNo=${reqPgNoInt1}">							
							<c:choose>
					...
					<td width="5%">
						<c:choose>
							<c:when test="${brd.file_cnt eq 1}">
								<a href="./brdNoticeTopFileDownload?fileNo=${brd.board_file_no}" title="바로다운로드">
