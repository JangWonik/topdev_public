...
<head>
	...
	<style type="text/css">
		...
		.comment {text-align:left;padding: 20px 20px 15px; line-height:195%;}
		.salary {font-weight: bold;}
		
		.innerTable td{padding: 5px 10px 5px; text-align: center;}
		.innerTable .innerHeader{padding: 0px 10px 0px; text-align: center; font-size:14px;}
		...
		.tg2 .comment {text-align:left;padding: 12px 20px 12px; line-height:180%;}
	</style>
	...
	<script>
		$(document).ready(function(){
			...
			if( "${mbrVo_Session.user_no}" == "${salDetail.user_no}"){
				...
				//170620 정민성이사님(38) -> 하명호이사님(966)
				if ( "${mbrVo_Session.user_no}" != 6 || "${mbrVo_Session.user_no}" != 966 || "${mbrVo_Session.user_no}" != 670 || "${mbrVo_Session.user_no}" != 1117 || "${mbrVo_Session.user_no}" != 215){
					alert("잘못된 접근입니다.");
					return;
				}
			}
			...
			$("#agreeBtn").click(function(){
				...
				var r = confirm("2017년도 연봉 금액 책정에 동의하십니까?");	
				if (!r){
					return false;
...
<div class="noPrint">
<input type="hidden" id="user_no" name="user_no" value="${salDetail.user_no}"/>
...
<center>
	<table class="tg">
		...
		<tr>
		    ...
		    <td class="header">${salDetail.user_name}</td><!-- 이름 -->
		    <td class="header">${salDetail.work_level}</td><!-- 직급 -->
		    <td class="header2" >생년월일</td>
		    <td class="header2" >${birth}</td><!-- 생년월일 -->
		</tr>
		
		<tr>
			<td class="header2" >주소</td>
		 	<td colspan="2" class="header2" > ${salDetail.home_address} </td><!-- 주소 -->
			<td class="header2" >전화번호</td>
			<td class="header2" >${salDetail.handphone}</td><!-- 전화번호 -->
	  	</tr>
	  	
	  	<tr>
   			<td colspan="7" style="padding-top:30px;padding-bottom:30px;text-align:center;">
   				위 사용자와 근로자는 아래의 연봉 및 근로조건을 성실히 이행할 것을 약정하고 연봉근로계약을 체결한다. <br />
   				<br/>
   				- 아 래 -
   			</td>
	  	</tr>
		...
		<tr>
	  	...
	  	<tr>
	    	...
	    	<c:choose>
	    		<c:when test="${salDetail.probation_date != null}">
	    			${salDetail.probation_date} ~ 2017년 12월 31일<br/>
	    		</c:when>
	    		
	    		<c:otherwise>
 				    <c:choose>
			    		<c:when test="${fn:substring(salDetail.join_date,0,4) > 2016  }">
		    				${salDetail.join_date} ~ 2017년 12월 31일<br/>
			    		</c:when>
			    		<c:otherwise>
			    			2017년 1월 1일 ~ 2017년 12월 31일<br/>
			    		</c:otherwise>
			    	</c:choose>
	    		</c:otherwise>
	    	</c:choose>
	    	...
	    	<td class="header">
	    		<font style="font-size:14px;"> ${salDetail.join_date} </font>
    		</td><!-- 입사일 -->
	  	</tr>
				...
				<center>
					 <table class="innerTable">
					 	<tr>
					 		...
					 		<td width="22%" class="salary">
						 		<font style="font-size:14px; line-height:120%;" class="salary">
						 			₩ <fmt:formatNumber value="${salDetail.sal_amt}" type="number"/><!-- 연봉총액 -->
						 		</font>
					 		</td>
					 		<td width="25%" style="line-height:100%">
					 			월 급여
					 			<br/><font style="font-size:11px;line-height:120%;">(연봉총액 / 13개월)</font>
					 		</td>
					 		<td width="20%" class="salary">
						 		<font style="font-size:14px;line-height:120%;">
						 			₩ <fmt:formatNumber value="${salDetail.sal_month}" type="number"/><!-- 월급여 -->
						 		</font>
					 		</td>
					 	</tr>
					 </table>
		...
		<tr>
			<td class="header" id="chkZone1">구성항목</td>
			<td colspan="4"  class="comment" >
				...
				<span>
					<br/>
					<br/>
					<font style="font-weight: bold; font-size: 17px"> 
						(동의자)
						${salDetail.user_name} 
					</font>
					<input type="checkbox" id="chk1"/>
					<span id="sign1" style="display:none;">
						<img src="${userSignPath}" width="50px" height="50px" />
					</span>
				</span> 
				<br/><br/>
				...
				<table class="innerTable">
					<tr>
						<td class="innerHeader">기본급</td>
				 		<td width="20%" class="salary">
				 			<font style="font-size:12px">₩</font> <fmt:formatNumber value="${salDetail.sal_basic}" type="number"/>
			 			</td>
				 		<td class="innerHeader">주휴수당</td>
				 		<td width="20%" class="salary">
							<font style="font-size:12px">₩</font> <fmt:formatNumber value="${salDetail.sal_weekrest}" type="number"/>
						</td>
				 	</tr>
				 	<tr>
				 		<td class="innerHeader">연장근로 및 가산수당</td>
				 		<td width="20%" class="salary">
				 			<font style="font-size:12px">₩</font> <fmt:formatNumber value="${salDetail.sal_adv}" type="number"/></td>
				 		<td class="innerHeader" style="line-height:100%">차량유지비 <br/>또는 교통비</td>
				 		<td width="20%" class="salary">
				 			<font style="font-size:12px">₩</font> <fmt:formatNumber value="${salDetail.sal_car}" type="number"/></td>
				 	</tr>
				 	<tr>
				 		<td class="innerHeader">식대</td>
				 		<td width="20%" class="salary">
				 			<font style="font-size:12px">₩</font> <fmt:formatNumber value="${salDetail.sal_food}" type="number"/></td>
				 		<td></td>
				 		<td></td>
				 	</tr>
				 	<tr>
				 		<td class="innerHeader">월 급여 합계</td>
				 		<td colspan="3" class="salary">
				 			<font style="font-size:12px">₩</font> <fmt:formatNumber value="${salDetail.sal_month}" type="number"/> </td>
				 	</tr>
				 </table>
			</td>
		</tr>
		...
		<tr>
			...
			<td colspan="5"  class="comment">
				...
				<br/>나. 근로자는 사용자가 승인한 경우를 제외하고 업무인수인계서를 작성하여 결재를 득하여야 하며, 
		...
		<tr>
			<td colspan="7" style="padding-top:30px;padding-bottom:0px;text-align:center;border-bottom: 0px;">
				${year}년 ${month}월 ${day}일<!-- 작성연월일 -->				
				<br/>						
				<br/>사용자와 근로자는 상기 사항에 대하여 이의가 없음을 확인하고 근로계약을 체결합니다.										
			</td>
		</tr>
		<tr>
			<td colspan="3" class="header2" style="border-right: 0; border-top: 0; padding-top: 0px;border-bottom: 0;">
				(사용자)&nbsp;&nbsp;&nbsp;<img src="./resources/ne_img/sign_salary.jpg" />
			</td>
			<td style="border: 0">
				<br/><br/><br/><br/><br/><br/><br/>
			</td>
			<td colspan="3" class="header2" style="border-left: 0; border-top: 0; padding-top: 0px;border-bottom: 0;">
				<font style="font-weight: bold; font-size: 17px"> 
					(근로자) ${salDetail.user_name} 
				</font>
				<input type="checkbox" id="chk2"/>
				<span id="sign2" style="display:none;">
					<img src="${userSignPath}" width="50px" height="50px" />
				</span>
			</td>
		</tr>
		...
		<tr>
			<td colspan="7" style="border-bottom: 0; border-top: 0;">
		...
		<tr>
			<td colspan="7" align="right" style="padding-right: 40px; font-size: 13px; border-top:0;">
				...
				<span id="sign3" style="display:none;">
					<img src="${userSignPath}" width="50px" height="50px" />
				</span>
				<br/><br/>
			</td>
		...
		<table>
			<tr>
				<td>
					...
					<c:if test="${userAgreeFlag == 1}">
						<input  type="button" id="agreeBtn" name="agreeBtn" value="작성완료" style="width:80px;height:40px;"/>
					</c:if>
					
					<c:if test="${userAgreeFlag == 2}">
...
<center>
	<table class="tg">
		<tr>
	   		<th colspan="7" style="font-size: x-large; font-weight:bold; padding-top: 20px;padding-bottom: 20px;">
	   			연봉 근로 계약서 
   			</th>
	  	</tr>
		...
		<tr>
	  	...
	  	<tr>
	    	...
	    	<td class="header">최초 입사일</td>
	    	<td class="header">${salDetail.join_date}</td><!-- 입사일 -->
	  	</tr>
	  	
		<tr>
			<td rowspan="4" class="header">임금</td>
			<td class="header">산정기간<br>지급방법<br>연봉총액<br>월 급여</td>
			<td colspan="4" align="left"  class="comment" style="line-height: 190%">
			 	가. 연봉금액의 변경시 새로운 연봉근로계약서를 작성하여 적용한다.
				<br/>나. 근로자의 연봉총액, 월 급여<font style="font-size:13px;">(연봉총액/13개월, 연봉총액의 12/13을</font>
				<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font style="font-size:13px;">연간 총 급여로 지급)</font>는 아래와 같으며 월 급여에서 법령 및 노사간 합의에 
				<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;의한 금액을 공제하고 매월 초일에 기산하여 말일에 마감하고 하고 
		...
		<tr>
			...
			<td colspan="4"  class="comment" style="line-height: 190%">
				월 급여의 구성항목은 아래와 같으며, 근로자는 사전에 아래와 같이 
				<br/>법정기준시간을 초과한 연장근로에 동의하며 월 급여에 연장근로시간 대한 
				<br/>임금 및 가산수당이 모두 포함되어 있음을 확인하였고, 근로자는 이와 같은 
				<br/>법정 제수당이 포함되어 있는 포괄임금산정방식에 동의한다. 
				...
				<table class="innerTable">
					<tr>
				 	...
				 	<tr>
				 		<td class="innerHeader">월 급여 합계</td>
				 		<td colspan="3" class="salary">
				 			<font style="font-size:12px">₩</font> <fmt:formatNumber value="${salDetail.sal_month}" type="number"/> 
			 			</td>
				 	</tr>
				 </table>
			</td>
