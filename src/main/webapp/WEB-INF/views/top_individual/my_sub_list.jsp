			...
			window.open('primBizRptDtl?suim_rpt_no='+suim_rpt_no,'tst','width='+sw+',height='+sh+',top='+mt+',left='+ml+',resizable=no,scrollbars=yes');
							...
							<div class= "tableStyle2">
								<table>
									...
									<tbody>
										<!--  -->
										<c:forEach items="${myNoSiteList}" var="suimVO" varStatus="status" >
										<c:set var="suimAcceptNo" value="${suimVO.suim_accept_no}"/>
										<c:set var="docType" value="${fn:substring(suimAcceptNo,0,5)}"/>										
										<c:choose>
											<c:when test='${docType eq "TLETC"}'>
												<tr onMouseOver="this.style.backgroundColor='#FFECEC'" onMouseOut="this.style.backgroundColor=''" style ="cursor:pointer;" onclick="popPrimBizDtl('${suimVO.suim_rpt_no}')">
													<!-- suim_rpt_type1 -->
											...
											<c:otherwise>
												<tr onMouseOver="this.style.backgroundColor='#FFECEC'" onMouseOut="this.style.backgroundColor=''" style ="cursor:pointer;" onclick="popSuimDtl('${suimVO.suim_rpt_no}')">
													<!-- suim_rpt_type1 -->
													<td>${suimVO.suim_rpt_type1_nm}</td>
													<!-- suim_accept_no -->
													<td>${suimVO.suim_accept_no}</td>
