...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
	...
	<script>
		...
		function reqPgNoClick4(pgNo){
			myNoMainPagingForm.reqPgNo.value = pgNo;
			myNoMainPagingForm.presentPage.value = "tmnointerimtab";
			myNoMainPagingForm.submit();
							...
							<div class= "tableStyle2">
								<table>
									...
									<tbody>
										<!--  -->
										<c:forEach items="${tmNoInterimList}" var="suimVO" varStatus="status" > <!-- begin="1" end="20" step="1" -->
										
										<tr onMouseOver="this.style.backgroundColor='#FFECEC'" onMouseOut="this.style.backgroundColor=''"
										style ="cursor:pointer;" onclick="popSuimDtl('${suimVO.suim_rpt_no}', '${suimVO.delDate}')">
											<!-- no -->
											<td>${totCntIntForListNo4 - (18 * (reqPgNoInt4 - 1)) - status.index}</td>
											<!-- lock -->
											<td>
	...
	<!-- page start -->
		<c:if test="${pgNm4 != null}">
			<p style="text-align:center;">
				<c:if test="${startPgNoInt4 > 10}">
					<a href="javascript:reqPgNoClick4(${startPgNoInt4 - 1});">&lt;</a>
				</c:if>
				<c:forEach var="pageNo" begin="${startPgNoInt4}" end="${endPgNoInt4}" step="1">
					<c:choose>
						<c:when test="${reqPgNoInt4 != pageNo}">
							<a href="javascript:reqPgNoClick4(${pageNo});"><c:out value="${pageNo}" /></a>
							&nbsp;&nbsp;
						</c:when>
						<c:otherwise>
				...
				<c:if test="${totPgCntInt4 >= endPgNoInt4 + 1}">
					<a href="javascript:reqPgNoClick4(${endPgNoInt4 + 1});">&gt;</a>
				</c:if>
			</p>
		</c:if>
	<!-- //page end -->
