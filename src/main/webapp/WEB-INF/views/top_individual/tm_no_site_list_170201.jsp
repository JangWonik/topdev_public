...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
	...
	<script>
		...
		function reqPgNoClick3(pgNo){
			myNoMainPagingForm.reqPgNo.value = pgNo;
			myNoMainPagingForm.presentPage.value = "tmnositetab";
			myNoMainPagingForm.submit();
							...
							<div class= "tableStyle2">
								<table>
									...
									<tbody>
										<!--  -->
										<c:forEach items="${tmNoSiteList}" var="suimVO" varStatus="status" > <!-- begin="1" end="20" step="1" -->
										
										<tr onMouseOver="this.style.backgroundColor='#FFECEC'" onMouseOut="this.style.backgroundColor=''"
										style ="cursor:pointer;" onclick="popSuimDtl('${suimVO.suim_rpt_no}', '${suimVO.delDate}')">
											<!-- no -->
											<td>${totCntIntForListNo3 - (18 * (reqPgNoInt3 - 1)) - status.index}</td>
											<!-- lock -->
											<td>
	...
	<!-- page start -->
		<c:if test="${pgNm3 != null}">
			<p style="text-align:center;">
				<c:if test="${startPgNoInt3 > 10}">
					<a href="javascript:reqPgNoClick3(${startPgNoInt3 - 1});">&lt;</a>
				</c:if>
				<c:forEach var="pageNo" begin="${startPgNoInt3}" end="${endPgNoInt3}" step="1">
					<c:choose>
						<c:when test="${reqPgNoInt3 != pageNo}">
							<a href="javascript:reqPgNoClick3(${pageNo});"><c:out value="${pageNo}" /></a>
							&nbsp;&nbsp;
						</c:when>
						<c:otherwise>
				...
				<c:if test="${totPgCntInt3 >= endPgNoInt3 + 1}">
					<a href="javascript:reqPgNoClick3(${endPgNoInt3 + 1});">&gt;</a>
				</c:if>
			</p>
		</c:if>
	<!-- //page end -->
