...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
	...
	<script>
		...
		function reqPgNoClick5(pgNo){
			myNoMainPagingForm.reqPgNo.value = pgNo;
			myNoMainPagingForm.presentPage.value = "mynointerim2tab";
			myNoMainPagingForm.submit();
							...
							<div class= "tableStyle2">
								<table>
									...
									<tbody>
										<!--  -->
										<c:forEach items="${myNoInterimList2}" var="suimVO" varStatus="status" > <!-- begin="1" end="20" step="1" -->
										
										<tr onMouseOver="this.style.backgroundColor='#FFECEC'" onMouseOut="this.style.backgroundColor=''"
										style ="cursor:pointer;" onclick="popSuimDtl('${suimVO.suim_rpt_no}', '${suimVO.delDate}')">
											<!-- no -->
											<td>${totCntIntForListNo5 - (18 * (reqPgNoInt5 - 1)) - status.index}</td>
											<!-- lock -->
											<td>
	...
	<!-- page start -->
		<c:if test="${pgNm5 != null}">
			<p style="text-align:center;">
				<c:if test="${startPgNoInt5 > 10}">
					<a href="javascript:reqPgNoClick5(${startPgNoInt5 - 1});">&lt;</a>
				</c:if>
				<c:forEach var="pageNo" begin="${startPgNoInt5}" end="${endPgNoInt5}" step="1">
					<c:choose>
						<c:when test="${reqPgNoInt5 != pageNo}">
							<a href="javascript:reqPgNoClick5(${pageNo});"><c:out value="${pageNo}" /></a>
							&nbsp;&nbsp;
						</c:when>
						<c:otherwise>
				...
				<c:if test="${totPgCntInt5 >= endPgNoInt5 + 1}">
					<a href="javascript:reqPgNoClick5(${endPgNoInt5 + 1});">&gt;</a>
				</c:if>
			</p>
		</c:if>
	<!-- //page end -->
