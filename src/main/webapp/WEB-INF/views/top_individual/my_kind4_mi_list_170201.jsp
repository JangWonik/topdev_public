...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
	...
	<script>
		...
		function reqPgNoClick6(pgNo){
			myNoMainPagingForm.reqPgNo.value = pgNo;
			myNoMainPagingForm.presentPage.value = "mynointerimtab";
			myNoMainPagingForm.submit();
							...
							<div class= "tableStyle2">
								<table>
									...
									<tbody>
										<!--  -->
										<c:forEach items="${myKind4MiList}" var="suimVO" varStatus="status" > <!-- begin="1" end="20" step="1" -->
										
										<tr onMouseOver="this.style.backgroundColor='#FFECEC'" onMouseOut="this.style.backgroundColor=''"
										style ="cursor:pointer;" onclick="popSuimDtl('${suimVO.suim_rpt_no}', '${suimVO.delDate}')">
											<!-- no -->
											<td>${totCntIntForListNo6 - (18 * (reqPgNoInt6 - 1)) - status.index}</td>
											<!-- lock -->
											<td>
	...
	<!-- page start -->
		<c:if test="${pgNm6 != null}">
			<p style="text-align:center;">
				<c:if test="${startPgNoInt6 > 10}">
					<a href="javascript:reqPgNoClick6(${startPgNoInt6 - 1});">&lt;</a>
				</c:if>
				<c:forEach var="pageNo" begin="${startPgNoInt6}" end="${endPgNoInt6}" step="1">
					<c:choose>
						<c:when test="${reqPgNoInt6 != pageNo}">
							<a href="javascript:reqPgNoClick6(${pageNo});"><c:out value="${pageNo}" /></a>
							&nbsp;&nbsp;
						</c:when>
						<c:otherwise>
				...
				<c:if test="${totPgCntInt6 >= endPgNoInt6 + 1}">
					<a href="javascript:reqPgNoClick6(${endPgNoInt6 + 1});">&gt;</a>
				</c:if>
			</p>
		</c:if>
	<!-- //page end -->
