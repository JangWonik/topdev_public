			...
			myNoMainPagingForm.submit();
							...
							<div class= "tableStyle2">
								<table>
									...
									<tbody>
										<!--  -->
										<c:forEach items="${myNoInterimList}" var="suimVO" varStatus="status" > <!-- begin="1" end="20" step="1" -->
										
										<tr onMouseOver="this.style.backgroundColor='#FFECEC'" onMouseOut="this.style.backgroundColor=''"
										style ="cursor:pointer;" onclick="popSuimDtl('${suimVO.suim_rpt_no}', '${suimVO.delDate}')">
											<!-- no -->
											<td>${totCntIntForListNo2 - (18 * (reqPgNoInt2 - 1)) - status.index}</td>
											<!-- lock -->
											<td>
		...
		<c:if test="${pgNm2 != null}">
			<p style="text-align:center;">
				<c:if test="${startPgNoInt2 > 10}">
					<a href="javascript:reqPgNoClick2(${startPgNoInt2 - 1});">&lt;</a>
				</c:if>
				<c:forEach var="pageNo" begin="${startPgNoInt2}" end="${endPgNoInt2}" step="1">
					<c:choose>
						<c:when test="${reqPgNoInt2 != pageNo}">
							<a href="javascript:reqPgNoClick2(${pageNo});"><c:out value="${pageNo}" /></a>
							&nbsp;&nbsp;
						</c:when>
						<c:otherwise>
				...
				<c:if test="${totPgCntInt2 >= endPgNoInt2 + 1}">
					<a href="javascript:reqPgNoClick2(${endPgNoInt2 + 1});">&gt;</a>
				</c:if>
			</p>
		</c:if>
	<!-- //page end -->
