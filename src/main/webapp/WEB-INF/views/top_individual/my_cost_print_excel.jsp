...
<%
	...
	response.setHeader("Content-Disposition", "attachment; filename=person_cost_statistics_"+pageContext.getAttribute("today")+".xls");

	//==========================================================================
    // 한글 파일명 처리 할경우 아래 사용
    //==========================================================================
	//String fileName = (String)request.getAttribute("user_name") + "_" + pageContext.getAttribute("today") + ".xls";
    //String convFileName = URLEncoder.encode(fileName, "UTF-8");
...
<body>
<h1>${user_name} 경비내역</h1>
<table cellpadding="0" cellspacing="0" width="100%" bordercolordark="white" bordercolorlight="#CCCCCC" border="1">
    <tr>
    ...
    <c:forEach items="${selAddCardList}" var="cardVO" varStatus="status">
        ...
        <c:if test="${(tmpAddFlag1 != tmpAddFlag2)}">
            <tr bgcolor="#E7ECF1">
                <td class="td" colspan="8" align="right"><b>소계</b></td>
                <td class="td" align="center"><fmt:formatNumber value="${tmpAddSum1}" pattern="###,###,##0.##"/> ea</td>
                <td class="td" align="right"><fmt:formatNumber value="${tmpAddSum2}" pattern="###,###,##0.##"/> 원</td>
            </tr>
            <c:set var="tmpAddFlag1" value="${cardVO.getCost_class_no()}" />
            <c:set var="tmpAddFlag2" value="${cardVO.getCost_class_no()}" />
    ...
    <tr bgcolor="#E7ECF1">
        <td class="td" align="right" colspan="8"><b>소계</b></td>
        <td class="td" align="center"><fmt:formatNumber value="${tmpAddSum1}" pattern="###,###,##0.##"/> ea</td>
        <td class="td" align="right"><fmt:formatNumber value="${tmpAddSum2}" pattern="###,###,##0.##"/> 원</td>
    </tr>

    <tr>
        <td class="td" colspan="8" bgcolor="#F8F8F8">
            <p align="right"><B><font color="black">합 계</font></B></p>
        </td>
        <td class="td">
            <c:choose>
    ...
    <c:set value="0" var="tmpAddFlagCash1"/>
    <c:set value="0" var="tmpAddFlagCash2"/>
    <c:set value="0" var="tmpAddFlagCash3"/>
    <c:forEach items="${selAddCashList}" var="cashVO" varStatus="status">
        <c:if test="${status.index == 0}">
            <c:set var="tmpAddFlagCash1" value="${cashVO.getCost_class_no()}" />
        </c:if>
        <c:set var="tmpAddFlagCash2" value="${cashVO.getCost_class_no()}" />
        <c:if test="${(tmpFlagCash1 != tmpFlagCash2)}">
            <tr bgcolor="#E7ECF1">
                <td class="td" colspan="8" align="right"><b>소계</b></td>
                <td class="td" align="center"><fmt:formatNumber value="${tmpAddSumCash1}" pattern="###,###,##0.##"/> ea</td>
                <td class="td" align="right"><fmt:formatNumber value="${tmpAddSumCash2}" pattern="###,###,##0.##"/> 원</td>
            </tr>
            <c:set var="tmpAddFlagCash1" value="${cashVO.getCost_class_no()}" />
            <c:set var="tmpAddFlagCash2" value="${cashVO.getCost_class_no()}" />
            <c:set var="tmpAddSumCash1" value="0" />
            <c:set var="tmpAddSumCash2" value="0" />
            <c:set var="tmpAddSumCash3" value="0" />
        </c:if>
        <tr>
            <td class="td"><p align="center">${cashVO.getCost_occur_date()}</p></td>
            <td class="td"><p align="center">${cashVO.getCost_reg_date()}</p></td>
            <td class="td">
                <c:if test="${cashVO.getCost_class_no() eq 11}"><p align="center">대중교통비</p></c:if>
                <c:if test="${cashVO.getCost_class_no() eq 21}"><p align="center">유류대</p></c:if>
                <c:if test="${cashVO.getCost_class_no() eq 31}"><p align="center">주차비</p></c:if>
        ...
        <c:set var="sum_cost_deposit_amt" value="${cashVO.cost_deposit_amt + sum_cost_deposit_amt}" />
        <c:set var="var_cost_aprv_state" value="${cashVO.cost_aprv_state}" />
        <c:set var="tmpAddSumCash1" value="${tmpAddSumCash1 + cashVO.getCost_bill_ea()}" />
        <c:set var="tmpAddSumCash2" value="${tmpAddSumCash2 + cashVO.getCost_pay_amt()}" />
        <c:set var="tmpAddSumCash3" value="${tmpAddSumCash3 + cashVO.cost_deposit_amt}" />
    </c:forEach>
    <tr bgcolor="#E7ECF1">
        <td class="td" align="right" colspan="8"><b>소계</b></td>
        <td class="td" align="center"><fmt:formatNumber value="${tmpAddSumCash1}" pattern="###,###,##0.##"/> ea</td>
    ...
    <c:forEach items="${selClaimCardList}" var="claimcardVO" varStatus="status">
        ...
        <c:if test="${(tmpClaimFlag1 != tmpClaimFlag2)}">
            <tr bgcolor="#E7ECF1">
                ...
                <td class="td" align="right"><fmt:formatNumber value="${tmpClaimSum2}" pattern="###,###,##0.##"/> 원</td>
            </tr>
            <c:set var="tmpClaimFlag1" value="${claimcardVO.getCost_class_no_claim()}" />
            <c:set var="tmpClaimFlag2" value="${claimcardVO.getCost_class_no_claim()}" />
        ...
        <tr>
            ...
            <td class="td" colspan="2"><p align="center"><c:out value="${claimcardVO.getCost_memo()}"/></p></td>
    ...
    <c:forEach items="${selClaimCashList}" var="claimcashVO" varStatus="status">
        ...
        <tr>
            ...
            <td class="td" colspan="2"><p align="center"><c:out value="${claimcashVO.getCost_memo()}"/></p></td>
    ...
    <tr bgcolor="#E7ECF1">
        ...
        <td class="td" align="right"><fmt:formatNumber value="${tmpClaimSumCash2}" pattern="###,###,##0.##"/> 원</td>
    </tr>
    <tr>
        <td class="td" colspan="8" bgcolor="#F8F8F8">
            <p align="right"><B><font color="black">합 계</font></B></p>
    ...
    <tr>
        <td class="td" colspan="8" bgcolor="#EDEDED">
...
<p>&nbsp;</p>
<p>▣ 협조건 내역</p>
<table align="center" cellpadding="0" cellspacing="0" width="100%" bordercolordark="white" bordercolorlight="#CCCCCC" border="1">
    <tr>
