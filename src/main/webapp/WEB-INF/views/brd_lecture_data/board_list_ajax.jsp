	...
	<table>							
		<c:choose>
			...
			<c:otherwise>
				...
				<tr>
					<td width="9%">${boardListCnt - ( 18 * (pageIdx-1) ) - status.index}</td>										
					<td width="58%" style="text-align:left;padding-left:10px;letter-spacing:0px;">
						<div style="width:400px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
						<a href="./lectureBoardDetail?boardNo=${brd.board_no}&boardSrcType=${searchVO.boardSrcType}&boardSrcWord=${searchVO.boardSrcWord}&titleSearch=${searchVO.titleSearch}&reqPgNo=${reqPgNoInt1}&boardType=${searchVO.boardType}">					
							${brd.title}							
						</a>
					...
					<td width="5%">
						<c:choose>
							<c:when test="${brd.file_cnt eq 1}">								
								<a href="./lectureTopFileDownload?fileNo=${brd.board_file_no}" title="바로다운로드">								
									<img src="./resources/ls_img/icon_disket.gif">
								</a>
							</c:when>
							<c:when test="${brd.file_cnt > 1}">
								<a href="./lectureTopFileDownload?fileNo=${brd.board_file_no}" title="총 ${brd.file_cnt} 개의 첨부파일이 존재합니다.">								
									<img src="./resources/ls_img/icon_disket.gif">
								</a>
							</c:when>
							<%-- <c:when test="${brd.file_cnt > 1}">
					...
					<td width="15%">${brd.user_nm}</td>
					<td width="15%">${brd.reg_date_fmt}</td>					
					<td width="8%">${brd.view_cnt}</td>
				</tr>
				</c:forEach>	
			</c:otherwise>
		</c:choose>		
