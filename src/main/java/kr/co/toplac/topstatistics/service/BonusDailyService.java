...
@Service
public class BonusDailyService {	
    ...
    public void syncProcess() {
    	...
    	logger.info("BonusDailyService Start : "+sBaseDate);
    	
    	//오늘날짜를 가지고 분기기준 날짜를 가져온다.
    	Map<String, Object> setupMap = DateUtil.makeQuarterDate(sBaseDate);
    	
    	//기준일을 넣어준다.
    	setupMap.put("base_date", sBaseDate);
    	
    	//근무율 계산 기준 시작일
    	String sDate = String.valueOf(setupMap.get("sDate"));					//조회 분기시작일
    	String eDate = String.valueOf(setupMap.get("eDate"));					//조회 분기종료일
    	String sBaseYear = String.valueOf(setupMap.get("baseYear"));		//기준년도
    	String sQuaterFlag = String.valueOf(setupMap.get("quaterFlag"));	//분기표시(1~4)    	
    	...
    	*/    	
    	...
    	nSchChk = sqlSession.selectOne("SuimSchedule.selectSchBonusDailyChk", sBaseDate);
    	...
    	if( nSchChk == 0 ) {		//성과급 스케줄러에 등록되지 않은경우만 작업한다.
    		...
    		for(int i=0; i < memberList.size(); i++) {
    			...
    			memberList.get(i).put("quater_flag",sQuaterFlag);   
    			...
    			nChk = DateUtil.compareDate(sDate, sJoinDate);					//입사일자와 분기 시작 기준일 비교
    			    			
    			if( nChk > 0 ) { 																	//입사일자가 분기 시작 기준일 이후인 경우 별도계산(최근 입사자) 
    			...
    			}else	if( nChk < 1 && workList.size() == 0 ) {		//입사일자가 분기시작 기준이 이전이면서 해당기간에 휴직일이 없는경우 (근무율을 100으로 넣어준다)
    		...
    		sqlSession.insert("SuimSchedule.insertSchBonusDaily", memberList);
    		//스케줄러 입력 끝
    		...
    		*/	
    		...
    		logger.info("BonusDaily Duplicated Error : "+sBaseDate);
    	}
    	
    	logger.info("BonusDailyService End : "+sBaseDate);
    }
}